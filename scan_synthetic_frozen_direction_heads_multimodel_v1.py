...
CUDA_VISIBLE_DEVICES=0 python scan_synthetic_frozen_direction_heads_multimodel_v1.py \
...
# Full 4-model x 2-dataset matrix:
CUDA_VISIBLE_DEVICES=0 python scan_synthetic_frozen_direction_heads_multimodel_v1.py \
...
SCRIPT_VERSION = "syn400-frozen-direction-head-multimodel-v1"
EPS = 1e-12
RELATIONS = ("left", "right", "above", "below")
REL_TO_ID = {r: i for i, r in enumerate(RELATIONS)}

MODEL_ORDER = ("qwen-3b", "qwen-7b", "llava-7b", "llava-13b")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=True,
        help="Also run ordinary greedy generation on target samples for baseline/W2C/C2W.",
    )
...
def extract_vectors(
    ...
    result = {
        ...
        "metadata": {**expected_meta, "n": int(len(relations)), "errors": len(errors)},
        "errors": errors,
    }
