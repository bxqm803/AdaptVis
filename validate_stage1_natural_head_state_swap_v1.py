...
"""
Stage-I natural head-state interchange intervention.

Question
========
Do the proposed Stage-I Direction heads only contain decodable spatial
information, or does the REAL head hidden state produced under the opposite
relation causally control downstream spatial computation?

This script does NOT ablate heads and does NOT construct an artificial
relation vector.

For every COCO sample it uses the SAME image and two query orders:

    target : A relative to B   -> relation r
    source : B relative to A   -> opposite(r)

At a tested attention head, the script captures the REAL pre-W_O head slices
at the object-token positions from the source/swapped branch.  It then reruns
the target/original branch and directly replaces that head's object states.

Main ROLE replacement:

    target subject A    <- source subject B
    target reference B  <- source reference A
...
Same-layer random heads receive the identical ROLE replacement as a second
control.

No target/source means are mixed and no source-target difference vector is
constructed.  The inserted vectors are actual head activations produced by
the model in the swapped branch.

Token alignment
===============
Object names can have different token counts.  --replacement-mode controls
how source object-token head states are mapped to target object-token slots:

    tokenwise_resample  (default)
        If counts match, copy token by token exactly.  Otherwise nearest-
        neighbor resample the source token-state sequence to target length;
        every inserted row is still an actual source head state.

    pooled_broadcast
        Replace every target object-token head state by the source object's
        pooled mean head state.

    mean_shift
        Shift the target token-state cloud so its pooled mean equals the
        source pooled mean, preserving target within-object token variation.
        This is less literal and is included only as a robustness check.

Main metric
===========
Frozen clean-trained relation probes read downstream decoder hidden states.
For target LEFT / source RIGHT, a downstream prediction of RIGHT is a
SOURCE-FOLLOW success.

The strongest evidence for a Stage-I head is:

    ROLE source-follow >> identity source-follow
    ROLE source-follow >> same-layer random source-follow

starting only at/after the patched layer and propagating into later object
hidden states.  Next-token and optional full-generation source-follow are
reported as downstream consequences, not required for intermediate causal
control.

Recommended full Top-10 run (all 440)
=====================================

CUDA_VISIBLE_DEVICES=0 python -u validate_stage1_natural_head_state_swap_v1.py \
  --model qwen-3b \
  --source-output-dir output/spatial_storage_transport_utilization/coco/qwen-3b \
  --heads 23:5,23:1,19:13,26:3,23:0,20:0,20:8,19:12,20:6,19:8 \
  --max-samples 0 \
  --pair-status all \
  ...
  --device cuda:0 \
  --no-run-generation \
  --output-dir output/qwen3b_stage1_natural_head_swap_top10_all440 \
  --overwrite
"""
...
VERSION = "stage1-natural-head-state-swap-v1"
...
def build_condition_replacements(
    *,
    ...
    if condition == "bundle_role":
        return {
            h: replacement_payload(source_state=source_states[h], alignment="role")
            for h in tested_heads
        }
    if condition == "bundle_identity":
        return {
            h: replacement_payload(source_state=source_states[h], alignment="identity")
            for h in tested_heads
        }
    ...
    if kind == "role":
        return {
            target_head: replacement_payload(
                source_state=source_states[target_head], alignment="role"
            )
        }
    if kind == "identity":
        return {
            target_head: replacement_payload(
                source_state=source_states[target_head], alignment="identity"
            )
        }
    if kind == "random_role":
        random_head = random_heads[index]
        payload = replacement_payload(
            source_state=source_states[random_head], alignment="role"
        )
        payload["alignment"] = "random_role"
        return {random_head: payload}
...
def main() -> None:
    ...
    try:
        ...
        if args.include_bundle:
            ...
            print("\n" + "=" * 112)
            print("BUNDLE NATURAL ROLE REPLACEMENT: OBJECT SOURCE-FOLLOW")
            print("=" * 112)
            print(
        ...
        report = [
            ...
            "  1) natural ROLE state replacement raises downstream OBJECT source-follow",
