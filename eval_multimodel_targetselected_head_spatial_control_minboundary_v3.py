...
# -*- coding: utf-8 -*-
"""
eval_multimodel_targetselected_head_spatial_control_minboundary_v3.py
...
EPS = 1e-12
SCRIPT_VERSION = "multimodel-targetselected-head-spatial-control-minboundary-v3"
MODEL_ORDER = (
    "qwen2-2b",
    "qwen-3b",
    "qwen-7b",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Optional absolute trust cap on one local residual step; 0 disables the cap.",
    )
    p.add_argument(
        "--boundary-backtrack-tries", type=int, default=24,
        help="Maximum damping trials for a local minimum-boundary step.",
    )
    p.add_argument(
        "--boundary-backtrack-shrink", type=float, default=0.5,
        help="Multiplicative damping factor when the local boundary step is non-finite or does not improve the target margin.",
    )
    p.add_argument(
        "--min-margin-improvement", type=float, default=1e-7,
        help="Minimum increase in the worst target-vs-competitor teacher-forced margin required to accept a damped local step.",
    )
    ...
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    if gen_pred == target:
        ...
        for step_idx in range(1, int(args.max_steps) + 1):
            ...
            raw_step_norm = float(np.sqrt(np.sum(flat_step * flat_step)))
            ...
            # The local linear model can badly overshoot when a margin gradient
            # is tiny.  Dampen only the *minimum-boundary direction* until the
            # exact nonlinear teacher-forced margin is finite and improves.
            # This is a numerical trust-region safeguard; it does not reintroduce
            # a natural-gap steering unit or a hand-chosen steering direction.
            current_worst_margin = float(np.min(margins))
            accepted = False
            accepted_scores: Optional[Dict[str, float]] = None
            accepted_step: Optional[np.ndarray] = None
            alpha = 1.0
            shrink = float(args.boundary_backtrack_shrink)
            if not (0.0 < shrink < 1.0):
                raise ValueError("--boundary-backtrack-shrink must be in (0,1)")

            for _trial in range(int(args.boundary_backtrack_tries)):
                trial_flat = alpha * flat_step
                trial_step = trial_flat.reshape(len(layers), 2)
                trial_coords = coords + trial_step
                try:
                    trial_scores = all_scores(
                        model=model, decoder_layers=decoder_layers, prepared=prepared,
                        candidate_ids=candidate_ids, layers=layers, geom=geom,
                        coords=trial_coords, reduction=args.sequence_score_reduction,
                    )
                    vals = np.asarray([trial_scores[r] for r in REL], dtype=np.float64)
                    finite = bool(np.all(np.isfinite(vals)))
                    trial_worst_margin = (
                        min(float(trial_scores[target]) - float(trial_scores[r]) for r in competitors)
                        if finite else float("-inf")
                    )
                except (RuntimeError, FloatingPointError, ValueError):
                    finite = False
                    trial_worst_margin = float("-inf")
                    trial_scores = None

                if finite and trial_worst_margin >= current_worst_margin + float(args.min_margin_improvement):
                    accepted = True
                    accepted_scores = dict(trial_scores)
                    accepted_step = trial_step
                    break
                alpha *= shrink

            if not accepted or accepted_step is None:
                stop_reason = "boundary_step_backtracking_failed"
                break

            coords = coords + accepted_step
            scores = accepted_scores if accepted_scores is not None else scores
            steps = step_idx
            last_step_norm = float(np.sqrt(np.sum(accepted_step * accepted_step)))
...
def run_pair(
    *, model_alias: str, dataset: str, model: Any, processor_or_backend: Any,
    ...
    meta = {
        ...
        "boundary_backtrack_tries": int(args.boundary_backtrack_tries),
        "boundary_backtrack_shrink": float(args.boundary_backtrack_shrink),
        "min_margin_improvement": float(args.min_margin_improvement),
