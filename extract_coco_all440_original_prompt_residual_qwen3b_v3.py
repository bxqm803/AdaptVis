...
# -*- coding: utf-8 -*-
"""
Extract ALL-COCO original-prompt object-token REAL/NoImage states and save REAL-NoImage residual vectors.

V3 fix: the exact original COCO question_text AND subject/reference strings are loaded
from the standard prompt JSONL, matching eval_nonoracle_spatial_consensus_gated_repair_v1.py.
No controlled/rewritten prompt is introduced.
...
Example:
    CUDA_VISIBLE_DEVICES=0 python extract_coco_real_noimage_residual_states_v1.py \
        --dataset coco_two \
        --data-root data \
        --model qwen-3b \
        --device cuda:0 \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--require-all",
        action="store_true",
        help="Fail unless every requested COCO sample succeeds in every requested condition.",
    )
    p.add_argument(
        "--keep-fp32",
        action="store_true",
...
def load_original_prompts(path: Path) -> Dict[int, Dict[str, Any]]:
    """Load the exact repo-standard COCO prompt metadata used by repair.

    IMPORTANT: subject/reference come from the prompt JSONL itself, not from
    dataset Record.subject/reference.  This matches gate.load_data() in the
    non-oracle repair pipeline exactly.
    """
    return cent.load_standard_prompts(path)


def build_chat_prompt_original(
    processor: Any,
    question_text: str,
    with_image: bool,
) -> str:
    ...
    *,
    ...
    for record in tqdm(records, desc=desc):
        try:
            ...
            prompt_meta = prompt_rows[int(record.sid)]
            question_text = str(prompt_meta["question_text"])
            prompt_subject = str(prompt_meta["subject"])
            prompt_reference = str(prompt_meta["reference"])
            prompt_relation = norm_relation(prompt_meta["answer_raw"])

            rendered = build_chat_prompt_original(
                processor,
                question_text,
            ...
            # CRITICAL: use the exact object surface strings parsed from the same
            # standard prompt JSONL that the downstream repair pipeline uses.
            subject_index = base.find_phrase_last_token(
                processor.tokenizer, input_ids, prompt_subject
            )
            reference_index = base.find_phrase_last_token(
                processor.tokenizer, input_ids, prompt_reference
            )
            ...
            subjects.append(prompt_subject)
            references.append(prompt_reference)
            relations.append(prompt_relation)
            ...
            errors.append(
                {
                    ...
                    "record_relation": record.relation,
                    "prompt_subject": (prompt_rows.get(int(record.sid), {}) or {}).get("subject", ""),
                    "prompt_reference": (prompt_rows.get(int(record.sid), {}) or {}).get("reference", ""),
                    "prompt_relation": (prompt_rows.get(int(record.sid), {}) or {}).get("answer_raw", ""),
                    "prompt_type": prompt_type,
                    "vision_mode": vision_mode,
...
def main() -> None:
    ...
    missing_prompts = [int(r.sid) for r in records if int(r.sid) not in prompt_rows]
    if missing_prompts:
        raise RuntimeError(
            f"Missing original prompts for {len(missing_prompts)} samples; "
            f"first={missing_prompts[:10]}"
        )
    ...
    try:
        ...
        missing_common = sorted(set(int(r.sid) for r in records) - set(map(int, sids)))
        (out_dir / "missing_common_sids.json").write_text(
            json.dumps(missing_common, indent=2), encoding="utf-8"
        )
        if args.require_all and missing_common:
            raise RuntimeError(
                f"--require-all requested but only {len(sids)}/{len(records)} samples "
                f"succeeded in every condition; missing={len(missing_common)}, "
                f"first={missing_common[:20]}. Inspect states/*.errors.json."
            )
        ...
        for prompt_type in prompts:
            ...
            for baseline_mode, suffix in [
                ...
                # Save the aligned residual representation so downstream causal
                # decomposition experiments can consume it directly.
                correct_data = condition_data[correct_name]
                sid_arr = np.asarray(correct_data["sample_index"], dtype=np.int64)
                sid_to_pos = {int(s): i for i, s in enumerate(sid_arr.tolist())}
                idx = np.asarray([sid_to_pos[int(s)] for s in sids], dtype=np.int64)
                residual_meta = {
                    "dataset": args.dataset,
                    "model_alias": args.model,
                    "repo_id": spec.repo_id,
                    ...
                    "source_correct": str(condition_npz_path(out_dir, prompt_type, "correct")),
                    "source_baseline": str(condition_npz_path(out_dir, prompt_type, baseline_mode)),
                    "decoder_blocks": len(layers),
                    "hidden_size": int(Xres.shape[-1]),
                    "n_saved": int(len(sids)),
                    "seed": args.seed,
                    "transformers_version": transformers.__version__,
                }
                residual_path = out_dir / "states" / f"{derived_name}.npz"
                atomic_save_npz(
                    residual_path,
                    {
                        "metadata_json": np.array(json.dumps(residual_meta), dtype=object),
                        "sample_index": np.asarray(sids, dtype=np.int64),
                        "image_id": np.asarray(correct_data["image_id"])[idx],
                        "subject": np.asarray(correct_data["subject"])[idx],
                        "reference": np.asarray(correct_data["reference"])[idx],
                        "relation": np.asarray(correct_data["relation"])[idx],
                        "decoder_block_index": np.asarray(layers, dtype=np.int32),
                        "relation_vectors": Xres.astype(np.float32 if args.keep_fp32 else np.float16),
                    },
                )
                print(f"[saved residual] {residual_path} | shape={Xres.shape}")
