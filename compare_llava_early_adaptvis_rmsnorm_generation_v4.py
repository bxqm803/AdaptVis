...
"""
compare_llava_early_adaptvis_rmsnorm_generation_v4.py
...
Example
-------
CUDA_VISIBLE_DEVICES=0 python compare_llava_early_adaptvis_rmsnorm_generation_v4.py \
  --model llava-7b \
  --dataset coco_two \
  --data-root data \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--skip-generation",
        action="store_true",
        help="Skip A/B/C autoregressive generation accuracy verification.",
    )
    p.add_argument(
        "--generation-only",
        action="store_true",
        help=(
            "Skip hidden-state extraction and run only A/B/C greedy generation. "
            "Useful after a previous run already completed the layer diagnostics."
        ),
    )
    p.add_argument(
...
def _manual_greedy_generate_tokens(
    model: Any,
    batch: Mapping[str, Any],
    *,
    ...
    """
    Deterministic autoregressive greedy decoding through the repository's
    custom LlavaForConditionalGenerationScal.forward().

    We do NOT call model.generate(): in newer Transformers, this custom class
    may not inherit GenerationMixin, so .generate() is absent.

    Step 0 is the full multimodal prompt. AdaptVis is applied there.
    Later steps use past_key_values and one generated token at a time.
    """
    ...
    if int(input_ids.shape[0]) != 1:
        raise RuntimeError("Manual greedy generation expects batch size 1.")
    ...
    # First/full-prompt forward. This is where visual tokens are merged and
    # where the AdaptVis last-query visual-logit intervention acts.
    with torch.inference_mode():
        outputs = model(
            ...
            return_dict=True,
            weight=weight,
            adjust_method=(attention_variant if weight is not None else None),
        )
    ...
    if past_key_values is None:
        raise RuntimeError(
            "LlavaForConditionalGenerationScal returned no past_key_values "
            "with use_cache=True."
        )
    ...
    # Cached decoding. No extra AdaptVis edit is required here: on the custom
    # cache path the image-token mask is None, so the intervention under study
    # is the full-prompt/last-token visual acquisition step.
...
def generate_relation_once(
    ...
    *,
    ...
    generated_ids = _manual_greedy_generate_tokens(
        model,
        batch,
        weight=weight,
...
def evaluate_generation_conditions(
    *,
    ...
    """Run actual greedy generation for A/B/C on exactly the same samples."""
    selected_sids = list(sids)
    if args.generation_max_samples is not None:
        selected_sids = selected_sids[: int(args.generation_max_samples)]

    rows: List[Dict[str, Any]] = []

    for sid in tqdm(selected_sids, desc="generation A/B/C"):
        sid = int(sid)
        rec = records_by_sid[sid]
        gt = str(labels_by_sid[sid])
        prompt = prompt_for_record(rec, prompt_map)
        ...
        try:
            ...
            set_rmsnorm_eps(model, args.base_rms_eps)
            ...
            set_rmsnorm_eps(model, args.enhanced_rms_eps)
            ...
            rows.append({
                ...
                "base_to_full_W2C": int(base_pred != gt and full_pred == gt),
                "base_to_full_C2W": int(base_pred == gt and full_pred != gt),
                "base_text": base_text,
    ...
    base_acc = safe_mean(row["base_correct"] for row in rows)
    eps_acc = safe_mean(row["eps_only_correct"] for row in rows)
    full_acc = safe_mean(row["full_correct"] for row in rows)
    w2c = sum(int(row["base_to_full_W2C"]) for row in rows)
    c2w = sum(int(row["base_to_full_C2W"]) for row in rows)

    return {
        "N": len(rows),
        "generation_backend": "manual greedy forward + past_key_values",
...
def main() -> None:
    ...
    # ---------------------------------------------------------------------
    # Optional generation-only fast path.
    # ---------------------------------------------------------------------
    if args.generation_only:
        ...
        labels_by_sid = {
            int(rec.sid): str(normalize_relation(rec.relation))
            for rec in records
        }
        ...
        with RestrictBoostLayers(
            ...
            generation_summary, generation_rows = evaluate_generation_conditions(
                model=model,
                processor=processor,
                records_by_sid=records_by_sid,
                sids=generation_sids,
        ...
        print("\n" + "=" * 166)
        print("ACTUAL GREEDY GENERATION ACCURACY CHECK")
        print("=" * 166)
        print(f"N={generation_summary['N']}")
        print("backend=manual greedy forward + past_key_values")
        print(
        ...
        print(
            ...
            f"{args.attention_variant}={args.boost_weight}, "
            f"L{boost_layers[0]}-L{boost_layers[-1]} : "
        ...
        print("=" * 166)
        print(f"[saved] {outdir / 'generation_summary.csv'}")
    ...
    # ---------------------------------------------------------------------
    # Actual autoregressive greedy generation verification.
    # ---------------------------------------------------------------------
    ...
    if not args.skip_generation:
        ...
        print("\n" + "=" * 166)
        print(
            "ACTUAL GREEDY GENERATION ACCURACY CHECK"
        )
        print("=" * 166)
        ...
        print(
            "backend=manual greedy forward + past_key_values "
            "(no GenerationMixin dependency)"
        )
