...
For EACH model independently:
  ...
  B) Minimum-residual spatial actuator
     ...
     4. Solve a local minimum-norm spatial step that raises the target above
        all three competitors to the current required decision margin.
     5. Apply the step and check ACTUAL autoregressive generation.  Crossing the
        relation-score boundary is NOT a stopping condition.  If generation is
        still wrong, deepen the required target margin, relinearize, and solve
        another minimum-norm spatial step.
     6. When generation first flips to the target, binary-search the final step
        segment to approximate the smallest generation-flipping intervention
        along that segment.
...
Teacher-forced relation scores guide the search but do not define success:
ACTUAL autoregressive generation is the only stopping criterion.  The optimized
norm is the actual relation-residual L2 norm inside the fitted spatial subspaces.
...
EPS = 1e-12
SCRIPT_VERSION = "multimodel-targetselected-head-spatial-control-genflip-v5"
MODEL_ORDER = (
    "qwen2-2b",
    "qwen-3b",
    "qwen-7b",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-steps", type=int, default=32, help="Maximum local relinearization / margin-deepening steps")
    p.add_argument(
        "--decision-margin-eps", type=float, default=1e-4,
        help="Target relation-score margin required over every competing relation.",
    )
    p.add_argument(
        "--generation-margin-increment", type=float, default=0.25,
        help=("If the target already beats all relation competitors but actual generation is still wrong, "
              "increase the required target margin by this amount and continue searching."),
    )
    p.add_argument(
        "--binary-search-steps", type=int, default=4,
        help="Binary-search iterations on the first generation-flipping spatial step; 0 disables refinement.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--max-residual-step", type=float, default=5000.0,
        help=("Trust-region cap on the L2 norm of one local residual-space update. "
              "The minimum-boundary QP still determines the direction/layer allocation; "
              "if its requested step is larger than this cap, only the capped step is "
              "taken and the model is relinearized. Set 0 to disable."),
    )
    p.add_argument(
        "--generation-check-every", type=int, default=1,
        help="Compatibility flag. GenFlip v5 checks actual generation after every accepted spatial step.",
    )
    p.add_argument(
...
def run_pair(
    *, model_alias: str, dataset: str, model: Any, processor_or_backend: Any,
    ...
    print("Synthetic directions -> target GT selects head ID -> head prediction sets target -> minimum residual H/V step -> deepen score margin until ACTUAL generation flips -> trust-cap each local step -> binary-refine final step")
