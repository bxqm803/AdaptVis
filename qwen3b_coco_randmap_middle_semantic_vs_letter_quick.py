...
Key metrics
-----------
1) semantic_wrong targetFollow vs baselineOppFollow: mapping-invariant relation causality.
2) letter_wrong targetFollow vs the same baseline target: answer-symbol/logit causality at the SAME middle object state.
3) semantic_correct vs letter_correct W2C: which code is more useful for repair.
...
def fit_letter_centroids(train_items, train_maps, state_by_sid, layers_req):
    out = {L: {} for L in layers_req}
    for L in layers_req:
        for a in LETTERS:
            xs = []
            for m in train_items:
                ...
                if sid not in state_by_sid:
                    continue
                correct_letter = train_maps[sid][m["gt"]]
                if correct_letter == a:
                    xs.append(np.asarray(state_by_sid[sid][L], np.float32))
            if not xs:
                raise RuntimeError(f"No train states for answer letter {a} at L{L}")
            out[L][a] = np.mean(np.stack(xs, axis=0), axis=0).astype(np.float32)
    return out


def centered_letter_direction(letter_cent, target_letter):
    others = [letter_cent[a] for a in LETTERS if a != target_letter]
    return (letter_cent[target_letter] - np.mean(np.stack(others, axis=0), axis=0)).astype(np.float32)
...
def main():
    ...
    try:
        ...
        print("=" * 112)
        print("QWEN3B COCO RANDOM-MAPPING — MIDDLE SEMANTIC vs LETTER CAUSAL TEST")
        print("=" * 112)
        ...
        # Two orthogonalized labelings over the SAME randomized prompts/states:
        #   relation centroids: grouped by LEFT/RIGHT/ABOVE/BELOW
        #   letter centroids:   grouped by current correct A/B/C/D
        # Because mapping is randomized/balanced, relation and letter identity are deconfounded.
        cent = traj.fit_centroids(train, state_by_sid, layers_req)
        letter_cent = fit_letter_centroids(train, train_map, state_by_sid, layers_req)
        np.savez_compressed(
            out / "train_relation_centroids_randmap_raw_subref.npz",
            **{f"L{L}_{r}": cent[L][r] for L in layers_req for r in REL},
        )
        np.savez_compressed(
            out / "train_letter_centroids_randmap_raw_subref.npz",
            **{f"L{L}_{a0}": letter_cent[L][a0] for L in layers_req for a0 in LETTERS},
        )

        # Quick geometry diagnostic: do middle LETTER directions already align with LM-head decision axes?
        tok = processor.tokenizer
        W = model.lm_head.weight.detach().float().cpu().numpy()
        ids = {}
        ok_single = True
        for a0 in LETTERS:
            t = tok.encode(a0, add_special_tokens=False)
            if len(t) != 1:
                ok_single = False
                print(f"WARNING: {a0} tokenizes as {t}; skipping unembedding cosine diagnostic")
                break
            ids[a0] = t[0]
        if ok_single:
            u = {a0: W[ids[a0]] for a0 in LETTERS}
            du = {a0: u[a0] - np.mean(np.stack([u[b] for b in LETTERS if b != a0], axis=0), axis=0) for a0 in LETTERS}
            print("\nMiddle object-pair LETTER direction vs centered LM-head decision direction")
            for L in layers_req:
                vals=[]
                for a0 in LETTERS:
                    ld = centered_letter_direction(letter_cent[L], a0)
                    vals.append(cosine_np(ld, du[a0]))
                print(f"L{L:02d}: " + " ".join(f"{a0}={v:+.3f}" for a0,v in zip(LETTERS,vals)) + f" | mean={np.mean(vals):+.3f}")
        ...
        for L in layers_req:
            for alpha in alphas:
                for condition in ("semantic_correct", "semantic_wrong", "letter_correct", "letter_wrong"):
                    rows_this = []
                    for b in tqdm(baseline, desc=f"L{L} a{alpha:g} {condition}", leave=False):
                        ...
                        if condition == "semantic_correct":
                            ...
                            target_rel = gt
                            target_letter = mp[gt]
                            d = centered_letter_direction(letter_cent[L], target_letter)
                        elif condition == "letter_wrong":
                            target_rel = opp
                            target_letter = mp[opp]
                            d = centered_letter_direction(letter_cent[L], target_letter)
                        else:
        ...
        (out / "metadata.json").write_text(json.dumps({
            ...
            "main_readout": "compare semantic vs letter steering under identical randomized mappings; semantic target tracks relation mapping, letter target directly tracks answer symbol",
        }, indent=2), encoding="utf-8")

        print("Saved:", out)
        print("Main thing to inspect: semantic_wrong vs letter_wrong targetFollow, plus semantic_correct vs letter_correct W2C.")
