...
def parse_args():
    ...
    # only_on: only evaluate gold=on samples
    # all: evaluate all four labels
...
def build_random_template3_prompt(
    ...
    letters = ["A", "B", "C", "D"]
    letter_to_label = {letter: label for letter, label in zip(letters, shuffled)}

    option_text = " ".join(
        [f"{letter}. {letter_to_label[letter]}" for letter in letters]
    )
...
def main():
    ...
    for local_idx in tqdm(range(start, end), desc="randomized-template3"):
        ...
        prompt_text, letter_to_label = build_random_template3_prompt(raw_question, args, rng)
