...
This restoration variant supports two directions:
  damage_on_original: original run receives swapped sender-bundle path states.
  restore_on_swapped: swapped run receives original sender-bundle path states.
The second direction tests whether the identified path is locally sufficient to
recover the original fixed-axis relation margin.
...
SCRIPT_VERSION = "coco-ioi-joint-bundle-path-restore-v1"
...
CONDITIONS = ("damage_on_original", "restore_on_swapped")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--conditions",
        default="restore_on_swapped",
        help="Comma-separated subset of damage_on_original,restore_on_swapped.",
    )
...
def receiver_channel_positions_for_side(
    pair: Any,
    channel: str,
    kv_scope: str,
    side: str,
) -> List[int]:
    if side not in {"original", "swapped"}:
    ...
    if side == "original":
        ...
        if channel == "q":
            return [int(pair.swapped_prompt_last)]
        if kv_scope == "objects":
            return list(map(int, pair.swapped_object_positions))
        if kv_scope == "all":
            return list(range(len(pair.swapped_ids)))
    raise ValueError(kv_scope)


def sender_position_mapping_for_condition(
    pair: Any,
    scope: str,
    condition: str,
) -> Tuple[str, str, Dict[int, int]]:
    """Return target_side, source_side, and target_position -> source_position.

    damage_on_original uses the existing corrupt direction:
      target original positions receive swapped source positions.

    restore_on_swapped reverses the direction:
      target swapped positions receive original source positions.
    """
    if condition == "damage_on_original":
        return "original", "swapped", dict(
            ioi_sender_position_mapping(pair, scope)
        )
    ...
    if scope == "prompt_last":
        return "swapped", "original", {
            int(pair.swapped_prompt_last): int(pair.original_prompt_last)
        }
    if scope == "objects_identity":
        ...
        return "swapped", "original", {
            int(pair.swapped_a_positions[-1]): int(pair.original_a_positions[-1]),
            int(pair.swapped_b_positions[-1]): int(pair.original_b_positions[-1]),
        }
    if scope == "objects_role":
        if not pair.original_a_positions or not pair.original_b_positions:
            raise RuntimeError("Missing object positions")
        return "swapped", "original", {
            int(pair.swapped_b_positions[-1]): int(pair.original_a_positions[-1]),
            int(pair.swapped_a_positions[-1]): int(pair.original_b_positions[-1]),
        }
    if scope == "all":
        ...
        return "swapped", "original", {
            index: index for index in range(len(pair.swapped_ids))
        }
    raise ValueError(scope)


def ioi_sender_position_mapping(pair: Any, scope: str) -> Dict[int, int]:
    # Defined as a small wrapper so sender_position_mapping_for_condition can be
    # parsed before the ioi module is imported in main. The actual function is
    # attached at runtime.
    raise RuntimeError("ioi_sender_position_mapping has not been initialized")


@torch.inference_mode()
def run_bundle_c_pass_side(
    *,
    bundle: Bundle,
    sender_mapping: Mapping[int, int],
    receiver_units: Sequence[Any],
    ...
    freeze_positions = set(map(int, sender_mapping.keys()))
    for unit in receiver_units:
        positions = receiver_channel_positions_for_side(
            pair, unit.channel, kv_scope, target_side
        )
        positions_by_layer_channel.setdefault((int(unit.layer), str(unit.channel)), []).extend(
            positions
        )
    ...
    positions_by_layer = {
        int(layer): sorted(freeze_positions)
        for layer in target_capture.layers
    }
    freeze = FreezeAttentionBundleAtPositions(
        decoder_layers=decoder_layers,
        attention_helper=attention_helper,
        receiver_module=receiver_module,
        output_projection_fn=ioi.output_projection_module,
        original=target_capture.attention,
        positions_by_layer=positions_by_layer,
        senders=bundle.heads,
        sender_mapping=sender_mapping,
        swapped=source_capture.attention,
    )
    projection_capture = ioi.CaptureProjectionAtPositions(
        decoder_layers=decoder_layers,
        attention_helper=attention_helper,
...
def summarize(rows: Sequence[Mapping[str, Any]], ioi: Any) -> List[Dict[str, Any]]:
    ...
    for (condition, bundle, receiver_unit, channel, scope), values in groups.items():
...
def main() -> None:
    ...
    globals()["ioi_sender_position_mapping"] = ioi.sender_position_mapping
    ...
    try:
        ...
        completed = {
            (
                int(row["sid"]),
                str(row.get("condition", "damage_on_original")),
                str(row["bundle"]),
                str(row["receiver_unit"]),
                str(row["sender_position_scope"]),
            )
        ...
        for source_row in rows_to_run:
            ...
            has_pending = any(
                (sid, condition, bundle.name, unit.unit, scope) not in completed
        ...
        print(
            ...
            f"receiver={unit.unit}, scopes={scopes}, conditions={conditions}",
            flush=True,
        )
        ...
        for sample_index, source_row in enumerate(
            ...
            try:
                ...
                for condition in conditions:
                    for scope in scopes:
                        target_side, source_side, mapping = sender_position_mapping_for_condition(
                            pair, scope, condition
                        )
                        if target_side == "original":
                            original_positions.update(map(int, mapping.keys()))
                        else:
                            swapped_positions.update(map(int, mapping.keys()))
                        if source_side == "original":
                            original_positions.update(map(int, mapping.values()))
                        else:
                        ...
                        original_positions.update(
                            map(
                                int,
                                receiver_channel_positions_for_side(
                                    pair, unit.channel, args.receiver_kv_scope, "original"
                                ),
                            )
                        )
                        swapped_positions.update(
                            map(
                                int,
                                receiver_channel_positions_for_side(
                                    pair, unit.channel, args.receiver_kv_scope, "swapped"
                                ),
                            )
                        )
                ...
                for condition in conditions:
                    for scope in scopes:
                        ...
                        if target_side == "original":
                            target_batch = pair.original_batch
                            target_capture = original_capture
                            target_baseline_margin = original_margin
                        else:
                            target_batch = pair.swapped_batch
                            target_capture = swapped_capture
                            target_baseline_margin = swapped_margin

                        source_capture = original_capture if source_side == "original" else swapped_capture

                        for bundle in bundles:
                            key = (int(pair.sid), condition, bundle.name, unit.unit, scope)
                            if key in completed:
                                continue
                            c_states = run_bundle_c_pass_side(
                                bundle=bundle,
                            ...
                            intervention = run_d_receiver_patch_side(
                                unit=unit,
                                c_states=c_states,
                                pair=pair,
                                target_side=target_side,
                            ...
                            if condition == "damage_on_original":
                                ...
                                normalized_metric = "normalized_damage"
                            elif condition == "restore_on_swapped":
                                raw_effect = float(intervention_margin - swapped_margin)
                                crossed = bool(swapped_margin < 0 <= intervention_margin)
                                normalized_metric = "normalized_recovery"
                            else:
                                raise ValueError(condition)
                            normalized_effect = float(raw_effect / denominator)
                            row = {
                                "script_version": SCRIPT_VERSION,
                                "phase": "bundle_path_restoration",
                                "condition": condition,
                                "target_side": target_side,
                                ...
                                "path_definition": "joint_sender_bundle_to_receiver_through_residual_and_mlps_only",
                                "intermediate_attention": f"frozen_to_{target_side}",
                                "mlps": "recomputed",
                                "score_mode": "next_token_relation_variants_at_prompt_last",
                                "original_margin": original_margin,
                                "swapped_margin_fixed_axis": swapped_margin,
                                "target_baseline_margin_fixed_axis": target_baseline_margin,
