...
# -*- coding: utf-8 -*-
"""
Behavioral test for relation-free, sample-specific middle-token selection.

Question
--------
Do the tokens selected from each sample's OWN relation-free internal signals
actually repair that sample's generation?

This script does NOT use LEFT/RIGHT/ON/UNDER to select deployable methods.
GT relation is used only AFTER generation to score accuracy.

Inputs
------
1) Existing oracle L20..L26 K36 run:
     <run-dir>/baseline.csv
     <run-dir>/mediation_tokens.csv
     <run-dir>/selected_tokens.csv

2) Existing self-predict feature run:
     <feature-dir>/all_candidate_self_features.pkl.gz

No attention-feature recomputation is needed.  For each sample we only run:
  - one clean REAL forward and one GRAY forward to recover the sample's
    Real-Gray delta_h at L20..L26;
  - edited generations for the requested selection methods.

Default first-pass sweep
------------------------
Core targets:
  core50 -> fixed K=median oracle core50 size (~7)
  core70 -> fixed K=median oracle core70 size (~14)
  core80 -> fixed K=median oracle core80 size (~19)

Deployable / relation-free methods:
  attn_delta_max
      position score = max_L rank(next-layer attention delta)
      edit layer     = argmax_L same score

  attn_delta_top2
      position score = mean(top-2 layer scores)
      edit layer     = argmax_L same score

  attn_delta_mean
      position score = mean over L20..L26
      edit layer     = argmax_L same score

  attn_posdelta_max
      position score = max_L raw max-positive-head attention delta
      edit layer     = argmax_L same score

  attn_ensemble
      position score = max_L attention-ensemble score
      edit layer     = argmax_L same score

  attn75_prior25
      75% current-sample attention + 25% leave-one-sample-out
      P(core | layer, broad_role), NO relation labels.

  attn_delta_top2_pm1split
      same position selector as attn_delta_top2, but edit peak layer +/-1.
      Total alpha for a selected position is split across available window
      layers, so this does not simply triple intervention magnitude.

Control:
  random_pos_self_layer
      random positions, but choose each random position's layer using the
      same self attention score.  This tests whether POSITION selection matters.

Diagnostics (oracle; NOT methods):
  oracle_core_exact
      true Core50/70/80 exact (layer,position), alpha=1 only by default.

  oracle_pos_self_layer
      true oracle core POSITIONS, but layer chosen only by self attention.
      Tests how much layer assignment costs.

  self_pos_oracle_layer
      self-selected positions, but each position's layer is chosen by maximum
      oracle mediation.  Tests how much position selection costs.

Outputs
-------
behavior_summary.csv
behavior_by_relation.csv
behavior_per_sample.csv
selected_positions.csv
analysis_summary.txt
per-sample cache under generation_chunks/

The script is resumable. If interrupted, rerun WITHOUT --overwrite.

Recommended first pass
----------------------
CUDA_VISIBLE_DEVICES=0 python -u eval_self_selected_core_behavior_v1.py \
  ...
  --alphas 1.0 \
  --diagnostic-alpha 1.0 \
  --output-dir output/qwen3b_coco_L20_26_self_core_behavior \
  --overwrite
"""
