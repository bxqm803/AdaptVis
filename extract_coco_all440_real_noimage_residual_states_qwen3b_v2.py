...
# -*- coding: utf-8 -*-
"""
Extract ALL COCO_two object-token REAL/NoImage states with a controlled prompt and save REAL-NoImage residual vectors.

This all-440 variant deliberately uses the same controlled prompt for every sample, with explicit final subject/reference mentions, so token-span extraction does not depend on heterogeneous original per-sample prompt wording.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--require-all",
        action="store_true",
        help="Fail unless every requested sample succeeds in every requested condition.",
    )
    return p.parse_args()
    ...
    *,
    ...
    def save_progress() -> None:
        ...
        metadata = {
            ...
            "prompt_template": "controlled four-way prompt with explicit final object mentions",
            "prompt_source": "internal prompt_text()",
    ...
    for record in tqdm(records, desc=desc):
        try:
            ...
            # Robust all-sample protocol: use one controlled prompt for every sample.
            # Explicit final object mentions guarantee a stable token locator.
            question_text = prompt_text(prompt_type, record.subject, record.reference)
...
def main() -> None:
    ...
    # This all-440 protocol does not depend on the heterogeneous original
    # per-sample prompt rows. Keep an empty mapping only for signature compatibility.
    prompt_rows: Dict[int, str] = {}
    ...
    try:
        ...
        requested_sids = sorted(int(r.sid) for r in records)
        missing_common = sorted(set(requested_sids) - set(map(int, sids)))
        if missing_common:
            miss_path = out_dir / "missing_common_sids.json"
            miss_path.write_text(json.dumps(missing_common, indent=2), encoding="utf-8")
            print(f"[common] missing={len(missing_common)}; saved={miss_path}; first={missing_common[:20]}")
            if args.require_all:
                raise RuntimeError(
                    f"--require-all requested but only {len(sids)}/{len(records)} succeeded. "
                    f"Inspect states/*.errors.json and {miss_path}."
                )
        ...
        summary: Dict[str, Any] = {
            "config": {
                ...
                "prompt_jsonl": None,
                "prompt_protocol": "controlled_all440_explicit_object_mentions",
                "model": args.model,
