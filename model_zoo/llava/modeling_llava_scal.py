...
class LlavaForConditionalGenerationScal(LlavaPreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(
        ...
        if image_to_overwrite.sum() != image_features.shape[:-1].numel():
            raise ValueError(
                f"The input provided to the model is wrong. "
                f"The number of image tokens is {torch.sum(special_image_token_mask)} "
                f"while the number of images is {num_images}. "
    ...
    def forward(
        ...
        """
        Main-style behavior:
            - In prefill, image_id is generated after merging image features.
            - In decode, image_id is None.
            - Do NOT fall back to external keys in decode.

        The arguments `keys` and `object_patch_mask` are kept in the signature
        only for compatibility with existing caller code. They are intentionally
        not used to continue AdaptVis during autoregressive decode.
        """
        ...
        # IMPORTANT:
        # This is the main-style behavior.
        # Do not use:
        #     effective_keys = image_id if image_id is not None else keys
        #
        # In decode steps image_id is None, so keys=None is passed to the
        # language model. This prevents AdaptVis from continuing to intervene
        # at every autoregressive decoding step.
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Main-style generation inputs:
        # Do not preserve keys / weight / adjust_method / object_patch_mask here.
        # The custom greedy search may still pass weight/adjust_method explicitly,
        # but forward() will only use image_id generated in prefill, not fallback keys.
        model_inputs.update(
            {
