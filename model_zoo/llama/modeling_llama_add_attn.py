...
def _apply_adaptvis_pre_softmax_variant(
    ...
    posneg_mean_img / posneg_median_img:
      sign-aware, per-head adaptive shrinkage.
      For positive image logits, subtract a positive statistic.
      For negative image logits, subtract a negative statistic, which moves them upward.
      This directly tests whether "moving image logits toward zero" explains the gain.
    """
    ...
    for b in range(bsz):
        ...
        for q in query_indices:
            ...
            if variant == "mul_img":
                ...
                #   weight=0.5 -> shrink toward zero
                #   weight=1.5 -> expand away from zero
                #
                # Extra strength:
...
class LLaMAAttention(nn.Module):
    ...
    def forward(
        ...
        # generate() may drop custom kwargs like `weight` before LLaMAAttention
        # receives them. For object-box / patch-id experiments, the runner sets
        # ADAPTVIS_WEIGHT per sample; use it as a fallback.
        ...
        if os.environ.get("DEBUG_OBJECTBOX_FORWARD", "0") == "1" and idx in [0, 1, 2, 3, 4]:
            print(
                f"[DEBUG FORWARD] idx={idx} q_len={q_len} kv_seq_len={kv_seq_len} "
                f"selected={_adaptvis_layer_selected(idx)} keys_none={keys is None} "
                f"weight={effective_weight} raw_weight={weight} "
                f"square={attn_weights.size()[2] == attn_weights.size()[3]} "
                f"patch_mode={os.environ.get('ADAPTVIS_PATCH_ID_MODE')} "
                f"n_patch_ids={len(os.environ.get('ADAPTVIS_PATCH_IDS', '').split(',')) if os.environ.get('ADAPTVIS_PATCH_IDS', '') else 0} "
                f"save_img_logits={SAVE_IMG_LOGITS} "
                f"save_path={os.environ.get('SAVE_IMG_LOGITS_PATH')}"
            )
