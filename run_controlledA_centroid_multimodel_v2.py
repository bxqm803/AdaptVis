#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run the Controlled-A centroid comparison across models.

This is the companion runner for:
    analyze_controlledA_centroid_step1_v2.py
...
It reports three centroid metrics:
...
All three use the same original/swap-aligned average protocol. The model is
frozen. The best layer/head is selected using full-set GT, so these are
diagnostic oracle statistics.

This runner intentionally does not report generation from Step 1. That trace
may force two decoding steps to expose answer-token routing. Use the separate
normal-generation run for a correct generation baseline.
"""
...
SCRIPT_VERSION = "controlledA-centroid-multimodel-v2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional explicit Controlled-A extractor module. Otherwise try "
            "extract_controlled_relation_states_standalone and "
    ...
    parser.add_argument(
        "--step1-script",
        default="analyze_controlledA_centroid_step1_v2.py",
    )
    ...
    parser.add_argument(
        "--output-root",
        default="output/controlledA_centroid_multimodel_v2",
    )
...
def import_controlled_module(explicit: str = "") -> Any:
    names: List[str] = []
    if explicit:
...
def select_best_positions(
    ...
    flat = int(np.nanargmax(per_head))
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if missing:
                raise RuntimeError(
                    ...
                    "This output must be produced by Step-1 v2."
                )
...
def evaluate_model(
    *,
    ...
    for index, row in enumerate(sample_rows, 1):
        ...
        similarity_ok = pred["similarity"] == gt
        headmean_ok = pred["headmean"] == gt
        best_head_ok = pred["best_head"] == gt

        counts["similarity"] += int(similarity_ok)
        counts["headmean"] += int(headmean_ok)
        counts["best_head"] += int(best_head_ok)

        relation_counts[gt]["n"] += 1
        relation_counts[gt]["similarity"] += int(similarity_ok)
        relation_counts[gt]["headmean"] += int(headmean_ok)
        relation_counts[gt]["best_head"] += int(best_head_ok)
...
def main() -> None:
    ...
    controlled_module = import_controlled_module(
        args.controlled_module
    )
    specs = merged_model_specs(controlled_module)
    ...
    print("\n" + "=" * 112)
    print("CONTROLLED-A CENTROID SUITE COMPLETE")
    print("=" * 112)
