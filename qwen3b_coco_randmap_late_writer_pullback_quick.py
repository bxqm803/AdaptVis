#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Qwen2.5-VL-3B + COCO random-map: late answer-writer onset + causal pullback.
...
Quick example
-------------
CUDA_VISIBLE_DEVICES=0 python -u qwen3b_coco_randmap_late_writer_pullback_quick.py \
  --writer-candidates 26,28,30,32,34,35 \
  --source-layers 14,18,22,26 \
  --max-samples 160 --onset-eval-max 32 --pullback-eval-max 24 \
  --output-dir output/qwen3b_coco_randmap_writer_pullback_quick --overwrite

If you already know the writer onset and want to skip the generation sweep:

CUDA_VISIBLE_DEVICES=0 python -u qwen3b_coco_randmap_late_writer_pullback_quick.py \
  --writer-layer 32 --source-layers 14,18,22,26 \
  --max-samples 160 --pullback-eval-max 32 \
  --output-dir output/qwen3b_coco_randmap_writer_pullback_L32 --overwrite
"""
...
def parse_args():
    ...
    p.add_argument("--writer-candidates", default="26,28,30,32,34,35",
    ...
    p.add_argument("--source-layers", default="14,18,22,26",
...
def main():
    ...
    try:
        ...
        print("=" * 116)
        print("QWEN3B COCO RANDOM-MAP — LATE ANSWER-WRITER ONSET + CAUSAL PULLBACK")
        print("=" * 116)
