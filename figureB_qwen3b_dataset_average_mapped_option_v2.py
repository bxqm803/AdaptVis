...
# -*- coding: utf-8 -*-
"""
Figure-B style dataset-average spatial steering -> mapped option.

Purpose
=======
Adapt Fig. 6B of "Linear Mechanisms for Spatiotemporal Reasoning in Vision
Language Models" to the AdaptVis relation->option setting.

For a fixed intervention layer L and a relation-specific spatial edit direction,
we ask whether steering toward a target spatial relation raises the logit of the
OPTION LETTER CURRENTLY ASSIGNED to that relation under each sample's randomized
A/B/C/D mapping.

Key idea
========
For each held-out sample i, relation r, and fixed layer L:

    target letter = pi_i(r) in {A,B,C,D}

Use TRAIN to fit per-layer relation centroids from the Real-Gray object-pair
state:

    q_L = (h_real_sub - h_real_ref) - (h_gray_sub - h_gray_ref).

Construct the target-vs-opposite spatial edit:

    d_{r,L} = mu_{r,L} - mu_{opp(r),L}

Apply subject/reference patch at layer L only:

    h_sub += 0.5 * alpha * d_{r,L}
    h_ref -= 0.5 * alpha * d_{r,L}

Then measure, for the CURRENT SAMPLE'S randomized mapping, the change in
next-step option logits:

    Delta logit(pi_i(r))
    Delta mean-logit(other 3 options)

Main outputs (5 figures)
========================
1) figureB_all_targets_aligned.png
...
Each plot shows:
  ...
  y-axis = average delta option logit relative to the unedited baseline
  curves = mapped target option / mean of other options
  shaded band = standard error of the mean
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Optional cap on TEST examples used for the dataset-average steering figure. 0 = all test.",
    )
    p.add_argument(
        "--test-filter",
        default="all",
        choices=["all", "correct", "wrong"],
        help="Optional filter based on the ORIGINAL unedited first-step prediction vs GT-mapped option.",
    )
...
def build_randmap_prompt(subject: str, reference: str, rel_to_letter: Mapping[str, str]) -> str:
    ...
    lines = [
        ...
        *(f"{a}. {letter_to_rel[a]}" for a in LETTERS),
...
def summarize_rows(rows: Sequence[Mapping[str, Any]], target_filter: Optional[str] = None) -> List[Dict[str, Any]]:
    by_alpha: Dict[float, List[Mapping[str, Any]]] = {}
    for r in rows:
        if target_filter is not None and r["target_relation"] != target_filter:
            continue
    ...
    for alpha in sorted(by_alpha):
        ...
        tgt = np.asarray([float(x["delta_target_logit"]) for x in chunk], np.float64)
        oth = np.asarray([float(x["delta_other_mean_logit"]) for x in chunk], np.float64)
        out.append({
            "target_relation": target_filter or "ALL",
            "alpha": alpha,
            "n": int(len(chunk)),
            "target_mean": float(tgt.mean()) if len(tgt) else float("nan"),
            "target_sem": float(tgt.std(ddof=1) / math.sqrt(len(tgt))) if len(tgt) > 1 else 0.0,
            "other_mean": float(oth.mean()) if len(oth) else float("nan"),
            "other_sem": float(oth.std(ddof=1) / math.sqrt(len(oth))) if len(oth) > 1 else 0.0,
        })
    return out
...
def main() -> None:
    ...
    try:
        ...
        print("=" * 100)
        print("FIGURE B — DATASET-AVERAGE SPATIAL STEERING -> MAPPED OPTION")
        print("=" * 100)
        ...
        # ------------------------------------------------------------------
        # 2) Dataset-average steering rows on TEST.
        # ------------------------------------------------------------------
        rows: List[Dict[str, Any]] = []
        kept = 0
        for m in tqdm(test, desc="TEST steering rows"):
            sid = int(m["sid"])
            ...
            try:
                ...
                q, subject_positions, reference_positions = collect_realgray_pair_states(
                    model=model,
                    processor=processor,
                    decoder_layers=decoder_layers,
                    rec=rec_by_sid[sid],
                ...
                for target_rel in REL:
                    target_letter = mapping[target_rel]
                    other_letters = [x for x in LETTERS if x != target_letter]
                    opposite_rel = OPP[target_rel]
                    direction = cent[a.layer][target_rel] - cent[a.layer][opposite_rel]
                    direction = np.asarray(direction, np.float32)

                    base_target = float(base_sc["letter_logits"][target_letter])
                    base_other_mean = float(np.mean([base_sc["letter_logits"][x] for x in other_letters]))
                    ...
                    for alpha in alphas:
                        ...
                        target_val = float(patched["letter_logits"][target_letter])
                        other_mean = float(np.mean([patched["letter_logits"][x] for x in other_letters]))
                        rows.append({
                            "sid": sid,
                            "gt_relation": m["gt"],
                            ...
                            "alpha": float(alpha),
                            "layer": int(a.layer),
                            "delta_target_logit": target_val - base_target,
                            "delta_other_mean_logit": other_mean - base_other_mean,
        ...
        write_csv(out / "figureB_rows.csv", rows)

        all_summary = summarize_rows(rows, None)
        per_target = {r: summarize_rows(rows, r) for r in REL}
        summary_rows: List[Dict[str, Any]] = []
        summary_rows.extend(all_summary)
        for r in REL:
        ...
        print("=" * 100)
        print(
            f"model={a.model} | layer=L{a.layer} | TEST kept={kept} | rows={len(rows)} | output={out}",
            flush=True,
        )
