#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Training-free centroid-guided autoregressive generation.

The script evaluates three signals on the same standard COCO/VG two-object
questions:

1) Baseline generation
   The frozen VLM generates normally with greedy decoding.

2) Centroid grounding
   At a selected decoder layer, each object-word hidden state is matched to all
   visual-token hidden states by cosine similarity.  The weighted visual
   centroids provide dx/dy and a direct four-way spatial prediction.

3) Centroid-guided generation
   The model still calls ``model.generate`` and still generates
   autoregressively.  Only at the first answer-token step, a label-free
   LogitsProcessor adds a soft bias to the token logits for
   left/right/above/below.  The bias is computed only from centroid dx/dy and
   label-free grounding confidence.  No model weights, probes, or per-sample GT
   labels are used by the guidance.

GT answers are read only for reporting accuracy after generation.
"""
...
SCRIPT_VERSION = "centroid-guided-generation-v1"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--layer",
        default="auto",
        help="Zero-based decoder block index used for centroid grounding, or 'auto'.",
    )
    ...
    p.add_argument(
        ...
        help="Confidence saved during centroid grounding.",
    )
    p.add_argument(
        "--guide-strength",
        type=float,
        default=4.0,
        help=(
            "Maximum additive logit bias. The effective value is multiplied by "
            "the selected label-free guidance confidence."
        ),
    )
    p.add_argument(
        "--guide-confidence-mode",
        default="separation_axis",
        choices=["none", "separation", "axis", "separation_axis"],
        help=(
            "Label-free confidence used to scale generation guidance. "
            "'separation_axis' multiplies map separation and axis confidence."
        ),
    )
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="Disable guidance below this confidence.",
    )
    p.add_argument(
        "--guide-mode",
        default="soft",
        ...
        help=(
            "soft: add bias while leaving the full vocabulary available; "
            "constrained: at the first answer step allow only one-token variants "
            "of left/right/above/below, with the same centroid bias."
        ),
    )
    ...
    p.add_argument(
        "--only",
        choices=["both", "baseline", "guided"],
        default="both",
        help="Run both passes, only baseline/capture, or only guided from saved centroids.",
    )
    return p.parse_args()
...
def geometry_from_patch(patch: PatchData) -> Tuple[np.ndarray, Dict[str, float]]:
    """Return normalized signed geometry scores for L/R/A/B."""
    dx = float(patch.delta_x)
    dy = float(patch.delta_y)
    if not np.isfinite(dx) or not np.isfinite(dy):
        raise ValueError(
            f"Centroid coordinates are unavailable for sid={patch.sid}: dx={dx}, dy={dy}"
        )

    raw = np.asarray([-dx, dx, -dy, dy], dtype=np.float32)
    ...
    return normalized, {
        ...
        "above": float(normalized[2]),
        "below": float(normalized[3]),
    }


def guidance_confidence(patch: PatchData, mode: str) -> float:
    ...
    raise ValueError(f"Unsupported guide confidence mode: {mode}")


class CentroidSpatialLogitsProcessor(LogitsProcessor):
    """Apply centroid-derived bias only at the first generated answer token."""
    ...
    def __init__(
        ...
        *,
        ...
        self.label_token_ids = {
            label: [int(x) for x in ids]
            for label, ids in label_token_ids.items()
        }
        self.label_bias = {
            label: float(label_bias[label])
            for label in RELATIONS
        }
        self.mode = str(mode)
        self.applied = False
    ...
    def __call__(
        ...
        if self.mode == "constrained":
            ...
            for label in RELATIONS:
                bias = self.label_bias[label]
                for token_id in self.label_token_ids[label]:
                    masked[:, token_id] = updated[:, token_id] + bias
            updated = masked
        elif self.mode == "soft":
            for label in RELATIONS:
                bias = self.label_bias[label]
                for token_id in self.label_token_ids[label]:
                    updated[:, token_id] = updated[:, token_id] + bias
        else:
            raise ValueError(f"Unsupported guide mode: {self.mode}")
...
def guided_generate(
    *,
    ...
    guide_confidence_mode: str,
    min_guide_confidence: float,
    guide_mode: str,
    max_new_tokens: int,
) -> Tuple[str, Dict[str, Any]]:
    input_length = int(batch["input_ids"].shape[1])
    if input_length != patch.input_length:
        raise RuntimeError(
            f"Input length changed: saved={patch.input_length}, current={input_length}"
        )

    geometry_vector, geometry_scores = geometry_from_patch(patch)
    confidence = guidance_confidence(patch, guide_confidence_mode)
    enabled = (
        confidence >= float(min_guide_confidence)
        and abs(float(guide_strength)) > 0.0
        and float(np.max(np.abs(geometry_vector))) > 0.0
    )
    effective_strength = float(guide_strength) * float(confidence)
    ...
    label_bias = {
        ...
        for index, label in enumerate(RELATIONS)
    }

    logits_processor = None
    spatial_processor = None
    if enabled or guide_mode == "constrained":
        spatial_processor = CentroidSpatialLogitsProcessor(
            prompt_length=input_length,
            label_token_ids=label_token_ids,
            label_bias=label_bias,
            mode=guide_mode,
        )
        logits_processor = LogitsProcessorList([spatial_processor])
    ...
    applied = bool(spatial_processor is not None and spatial_processor.applied)
    if spatial_processor is not None and not applied:
        raise RuntimeError("Centroid logits processor was registered but never applied")

    metadata = {
        "guidance_enabled": bool(enabled),
        "guidance_applied": applied,
        "guide_mode": guide_mode,
        "guide_confidence_mode": guide_confidence_mode,
        "guide_confidence": float(confidence),
        "guide_strength": float(guide_strength),
        "effective_guide_strength": float(effective_strength if enabled else 0.0),
        "geometry_scores": geometry_scores,
        "label_bias": label_bias,
        "delta_x": float(patch.delta_x),
        "delta_y": float(patch.delta_y),
        "map_separation": float(patch.map_separation),
        "axis_confidence": float(patch.axis_confidence),
    }
...
def summarize(
    ...
    baseline_valid = [r for r in base_by_sid.values() if r.get("prediction")]
    grounding_valid = [
        r for r in base_by_sid.values() if r.get("grounding_prediction")
    ]
    guided_valid = [r for r in guided_by_sid.values() if r.get("prediction")]

    common = sorted(set(base_by_sid) & set(guided_by_sid))
    paired = [
    ...
    for sid in paired:
        b = base_by_sid[sid]
        g = guided_by_sid[sid]
        changed += int(b.get("prediction") != g.get("prediction"))
        if (not b.get("correct")) and g.get("correct"):
            fixed += 1
        elif b.get("correct") and (not g.get("correct")):
            broken += 1
        elif b.get("correct") and g.get("correct"):
            both_correct += 1
        else:
            both_wrong += 1
        per_relation_sids[str(b.get("gt"))].append(sid)

    grounding_on_base_wrong = [
        row for row in grounding_valid
        if row.get("prediction") and not bool(row.get("correct"))
    ]
    grounding_on_base_correct = [
        row for row in grounding_valid
    ...
    agreement_rows = [r for r in grounding_valid if r.get("prediction")]
    agreement = (
        float(np.mean([
            r.get("prediction") == r.get("grounding_prediction")
    ...
    for relation, sids in sorted(per_relation_sids.items()):
        ...
        per_relation[relation] = {
            ...
            "guided_accuracy": accuracy(guided_by_sid[sid] for sid in sids),
        }

    paired_base_acc = accuracy(base_by_sid[sid] for sid in paired)
    paired_guided_acc = accuracy(guided_by_sid[sid] for sid in paired)

    return {
        "n_baseline_rows": len(base_by_sid),
        "n_guided_rows": len(guided_by_sid),
        ...
        "grounding_accuracy_on_baseline_wrong": accuracy(
            grounding_on_base_wrong, key="grounding_correct"
        ),
        "n_grounding_on_baseline_wrong": len(grounding_on_base_wrong),
        "grounding_accuracy_on_baseline_correct": accuracy(
            grounding_on_base_correct, key="grounding_correct"
        ),
        "n_grounding_on_baseline_correct": len(grounding_on_base_correct),
        "baseline_or_grounding_oracle_accuracy": oracle,
        "baseline_grounding_agreement": agreement,

        "paired_baseline_accuracy": paired_base_acc,
        "paired_guided_accuracy": paired_guided_acc,
        "paired_absolute_change": (
            paired_guided_acc - paired_base_acc
            if paired_base_acc is not None and paired_guided_acc is not None
            else None
        ),
        "fixed": fixed,
        "broken": broken,
        ...
        "mean_guide_confidence": (
            float(np.mean([
                float(guided_by_sid[sid].get("guide_confidence", 0.0))
                for sid in paired
            ]))
            if paired else None
        ),
        "mean_effective_guide_strength": (
            float(np.mean([
                float(guided_by_sid[sid].get("effective_guide_strength", 0.0))
                for sid in paired
            ]))
            if paired else None
        ),
...
def print_summary(summary: Dict[str, Any]) -> None:
    print("\n" + "=" * 88)
    print("BASELINE VS CENTROID GROUNDING VS CENTROID-GUIDED GENERATION")
    print("=" * 88)

    for index, (name, key, n_key) in enumerate([
        ("baseline generation", "baseline_accuracy", "n_baseline_valid"),
        ("centroid grounding", "grounding_accuracy", "n_grounding_valid"),
        ("guided generation", "guided_accuracy", "n_guided_valid"),
    ], 1):
    ...
    print(
        ...
        if agree is not None else "baseline/grounding agreement:  n/a"
    )

    pb = summary.get("paired_baseline_accuracy")
    pg = summary.get("paired_guided_accuracy")
    delta = summary.get("paired_absolute_change")
    print("\nPaired baseline/guided comparison:")
    print(f"paired valid:                 {summary.get('n_paired_valid')}")
    print(f"paired baseline:              {pb:.4f}" if pb is not None else "paired baseline: n/a")
    print(f"paired guided:                {pg:.4f}" if pg is not None else "paired guided: n/a")
    print(f"paired absolute change:       {delta:+.4f}" if delta is not None else "paired change: n/a")
    ...
    print(f"guided parse fail:            {summary.get('guided_parse_failures')}")
    print(f"mean guide confidence:        {summary.get('mean_guide_confidence')}")
    print(f"mean effective guide strength:{summary.get('mean_effective_guide_strength')}")
...
def main() -> None:
    ...
    if args.guide_strength < 0.0:
        raise ValueError("--guide-strength must be >= 0")
    if not (0.0 <= args.min_guide_confidence <= 1.0):
        raise ValueError("--min-guide-confidence must be in [0, 1]")
    ...
    if args.layer == "auto":
        if args.model not in AUTO_LAYERS:
            raise ValueError(f"No auto layer for {args.model!r}; pass --layer explicitly")
    ...
    patch_dir = output_dir / "centroids"
    patch_dir.mkdir(parents=True, exist_ok=True)

    baseline_path = output_dir / "baseline.jsonl"
    guided_path = output_dir / "guided.jsonl"
    ...
    run_config = {
        ...
        "guide_confidence_mode": args.guide_confidence_mode,
        "min_guide_confidence": args.min_guide_confidence,
        "guide_mode": args.guide_mode,
        "max_new_tokens": args.max_new_tokens,
        "n_records": len(records),
        "audit": audit,
        "uses_gt_for_guidance": False,
    ...
    if not (0 <= layer < len(decoder_layers)):
        raise ValueError(f"Requested layer={layer}, decoder has {len(decoder_layers)} layers")
    ...
    print("Guided label token map:")
    print(json.dumps(printable_token_map, ensure_ascii=False, indent=2))
    print(
        f"Guidance: mode={args.guide_mode}, strength={args.guide_strength}, "
        f"confidence={args.guide_confidence_mode}"
    )
    ...
    baseline_correct_count = sum(bool(row.get("correct")) for row in baseline_existing)
    guided_seen = len(guided_existing)
    guided_correct_count = sum(bool(row.get("correct")) for row in guided_existing)
    ...
    try:
                ...
                try:
                    ...
                    if should_print_sample(baseline_seen, args.print_every):
                        tqdm.write(
                            ...
                            f"dx={patch.delta_x:+.4f} dy={patch.delta_y:+.4f} | "
                            f"sep={patch.map_separation:.3f} axis={patch.axis_confidence:.3f}"
                        )
                    ...
                    tqdm.write(
                        f"\n[BASE ERROR] sid={sid} | {type(exc).__name__}: {exc}"
                    )
        ...
        if args.only in ("both", "guided"):
            print("\nPASS 2/2: centroid-guided autoregressive generation")
            baseline_by_sid = {
                int(row["sid"]): row
            ...
            fixed_running = 0
            broken_running = 0
            for row in guided_existing:
                sid = int(row["sid"])
                base = baseline_by_sid.get(sid)
                if base is None:
                    continue
                if (not base.get("correct")) and row.get("correct"):
                    fixed_running += 1
                elif base.get("correct") and (not row.get("correct")):
                    broken_running += 1

            for record in tqdm(records, desc=f"guided:{args.dataset}:{args.model}"):
                sid = int(record.sid)
                if sid in guided_done:
                    continue
                patch_path = patch_dir / f"{sid}.npz"
                if not patch_path.exists():
                    append_jsonl(
                        errors_path,
                        {
                            ...
                            "error": f"Missing centroid file: {patch_path}",
                        },
                    )
                    continue
                ...
                try:
                    ...
                    guided_text, guide_meta = guided_generate(
                        model=model,
                        processor=processor,
                        batch=batch,
                        patch=patch,
                    ...
                    append_jsonl(guided_path, row)
                    guided_done.add(sid)
                    guided_seen += 1
                    guided_correct_count += int(row["correct"])

                    base = baseline_by_sid.get(sid)
                    base_prediction = base.get("prediction") if base else None
                    status = "NO_BASELINE"
                    if base is not None:
                        if (not base.get("correct")) and row.get("correct"):
                            status = "FIXED"
                            ...
                            status = "BROKEN"
                            broken_running += 1
                        elif base.get("correct") and row.get("correct"):
                            status = "UNCHANGED_CORRECT"
                        else:
                            status = "UNCHANGED_WRONG"

                    if should_print_sample(guided_seen, args.print_every):
                        bias_text = ", ".join(
                            f"{label}={guide_meta['label_bias'][label]:+.3f}"
                            for label in RELATIONS
                        )
                        tqdm.write(
                            f"\n[GUIDE {guided_seen}/{len(records)}] sid={sid} | "
                            f"{subject} -> {reference}\n"
                            f"  question   : {one_line(question_text)}\n"
                            f"  gt         : {gt}\n"
                            ...
                            f"  correct    : {int(row['correct'])}\n"
                            f"  acc        : {guided_correct_count}/{guided_seen}="
                            f"{guided_correct_count / guided_seen:.4f}\n"
                            f"  base={base_prediction} -> guided={guided_prediction} | "
                            f"{status} | fixed={fixed_running} | broken={broken_running} | "
                            f"net={fixed_running - broken_running}\n"
                            f"  grounding={patch.grounding_prediction or None} | "
                            f"guide_conf={guide_meta['guide_confidence']:.3f} | "
                            f"effective_strength={guide_meta['effective_guide_strength']:.3f}\n"
                            f"  bias: {bias_text}"
                        )
                    ...
                    tqdm.write(
                        f"\n[GUIDE ERROR] sid={sid} | {type(exc).__name__}: {exc}"
                    )
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                finally:
        ...
        if baseline_rows and guided_rows:
            summary = summarize(baseline_rows, guided_rows)
