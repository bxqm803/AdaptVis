...
Path-level mediation test:
    ...
    upstream spatial state
        -> receiver-layer high-leverage text states
...
Instead, for an upstream spatial edit, define the spatial-induced receiver-layer change
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python -u \
  ...
  --random-repeats 1 \
  --output-dir output/qwen3b_spatial_to_L25_k1357_mediation_all440_v2 \
  --overwrite
...
EPS = 1e-12
SCRIPT_VERSION = "spatial-to-causaltoken-mediation-v2-multik"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ks", default="1,3,5,7", help="Comma-separated Top-K values, e.g. 1,3,5,7")
    p.add_argument(
        "--conditions",
...
# =============================================================================
# Receiver routing: manipulate ONLY the spatial-induced receiver-layer change
# =============================================================================
...
def summarize_effects(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["requested_k", "source_layer", "strength", "condition"]
    for (K, L, strength, cond), g in df.groupby(group_cols, sort=True):
    ...
    ret: List[dict] = []
    for (K, L, strength), g in out.groupby(
        ["requested_k", "source_layer", "strength"], sort=True
    ):
...
def summarize_by_relation(df: pd.DataFrame) -> pd.DataFrame:
    rows: List[dict] = []
    for (K, L, strength, cond, gt), g in df.groupby(
        ["requested_k", "source_layer", "strength", "condition", "gt"],
        sort=True,
    ):
        d = g["signed_effect"].to_numpy(dtype=float)
        rows.append({
...
def report_text(summary: pd.DataFrame, receiver_layer: int) -> str:
    lines: List[str] = []
    lines.append("=" * 150)
    lines.append(
        f"SPATIAL -> L{int(receiver_layer)} CAUSAL-TOKEN MEDIATION | D = own-logit(toward) - own-logit(away)"
    )
    lines.append("=" * 150)
    ...
    for K in sorted(summary["requested_k"].unique()):
        lines.append(f"[K={int(K)}]")
        kg = summary[summary["requested_k"] == K]
        for strength in sorted(kg["strength"].unique()):
            lines.append(f"  [strength={strength:g}]")
            sg = kg[kg["strength"] == strength]
            for L in sorted(sg["source_layer"].unique()):
                lines.append(f"  L{int(L):02d} -> receiver L{int(receiver_layer)}")
...
def main() -> None:
    ...
    if not ks or any(int(k) < 1 for k in ks):
    ...
    try:
        ...
        print(f"Ks={ks} positive_only={a.positive_only}")
        ...
        print(
            f"Receiver intervention restores ONLY spatial-induced L{receiver} changes "
            f"to the clean L{receiver} state; no full-state zeroing."
        )
        print(
        ...
        for m in tqdm(test, desc="Spatial->TopK mediation"):
            ...
            try:
                ...
                # Build Top-K and matched-random selections once per K.
                top_by_k: Dict[int, List[dict]] = {}
                random_by_k_repeat: Dict[Tuple[int, int], List[dict]] = {}

                for K in ks:
                    top_rows = ranked[: int(K)]
                    top_by_k[int(K)] = top_rows
                    ...
                    for rep in range(int(a.random_repeats)):
                        ...
                        random_by_k_repeat[(int(K), rep)] = rr
                        selection_rows.append({
                            "sid": sid,
                            "gt": DISPLAY[gt],
                            "receiver_layer": receiver,
                ...
                # Full is computed ONCE per source/strength and then copied
                # into each K row, avoiding redundant full-condition forwards.
                # -----------------------------------------------------
                for source_L in source_layers:
                    ...
                    for strength in strengths:
                        toward_coord = float(strength) * unit_coord
                        away_coord = -toward_coord

                        full_result = None
                        if "full" in conditions:
                            full_toward, full_restored_t = own_relation_logit(
                                model=model,
                                decoder_layers=decoder_layers,
                                prepared=prepared,
                                relation_token_ids=relation_token_ids,
                            ...
                            full_away, full_restored_a = own_relation_logit(
                                model=model,
                                decoder_layers=decoder_layers,
                                prepared=prepared,
                                relation_token_ids=relation_token_ids,
                        ...
                        for K in ks:
                            top_rows = top_by_k[int(K)]
                            top_positions = [
                                int(r["position"]) for r in top_rows
                            ]

                            condition_specs: List[
                                Tuple[str, int, List[int], Optional[str]]
                            ] = []
                            ...
                            for rep in range(int(a.random_repeats)):
                                random_positions = [
                                    int(r["position"])
                                    for r in random_by_k_repeat[(int(K), rep)]
                                ]
                            ...
                            # Replicate the same already-computed full result
                            # into each K slice for clean retention ratios.
                            if full_result is not None:
                                toward_logit, away_logit, restored_t, restored_a = full_result
                                effect_rows.append({
                                    "sid": sid,
                                    "gt": DISPLAY[gt],
        ...
        report = report_text(summary, receiver)
        print("\n" + report)
