...
SCRIPT_VERSION = "attention-flow-swap-step1-v1"
            ...
            try:
                ...
                if should_print_sample(
                    ...
                    tqdm.write(
                        ...
                        f"{subject} -> {reference}\n"
                        f"  GT/gen/swap-aligned: "
                        f"{gt} / {original['prediction']} / "
                        f"{swapped_aligned_generation}\n"
                        f"  answer swap consistent: "
                        f"{int(answer_swap_consistent)} | "
                        f"distribution cosine={distribution_cosine:.3f}\n"
                        f"  L{report_layer} similarity: "
                        f"{to_relation_name(original['similarity_prediction'][report])} "
                        f"-> swap "
                        f"{to_relation_name(similarity_swapped_pred[report])}\n"
                        f"  L{report_layer} attention head-mean: "
                        f"{to_relation_name(headmean_original[report])} "
                        f"-> swap "
                        f"{to_relation_name(headmean_swapped[report])}\n"
                        f"  answer routing subject/reference="
                        f"{row['report_first_answer_to_subject_mean']:.6f}/"
                        f"{row['report_first_answer_to_reference_mean']:.6f} | "
                        f"visual_mass="
                        f"{row['report_first_answer_visual_mass_mean']:.4f}"
                    )
