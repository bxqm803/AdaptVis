...
Cross-model / cross-dataset spatial-readout -> residual-spatial-control -> generation.
...
For EACH model independently:
  ...
  B) Residual spatial actuator
     1. Fit a 2D H/V object-pair spatial geometry ONLY from Synthetic-400.
     2. At a seven-layer mid/late window, edit only the subject/reference
        residual relation state:

            h_sub <- h_sub + delta_r / 2
            h_ref <- h_ref - delta_r / 2

        where delta_r lives in the fitted Synthetic H/V subspace.
     3. Optimize ONLY the 2*7 spatial coordinates toward the chosen control
        target, then run ACTUAL autoregressive generation.
...
The residual H/V geometry is source-only (Synthetic-400).  No hidden-space
vectors are shared across models: every model gets its own Synthetic reader
codebook and its own Synthetic residual geometry.
...
Synthetic residual control
==========================
Default --geometry-control gray uses:

  q_L = (h_sub^real - h_ref^real) - (h_sub^gray - h_ref^gray)

at each decoder block output.  This is uniform across all seven requested model
families and matches the reader's default Real-Gray visual dependence.
...
EPS = 1e-12
SCRIPT_VERSION = "multimodel-targetselected-head-spatial-control-v1"
MODEL_ORDER = (
    "qwen2-2b",
    "qwen-3b",
    "qwen-7b",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--step-natural", type=float, default=1.5)
    p.add_argument("--max-steps", type=int, default=60)
    p.add_argument("--line-search-shrink", type=float, default=0.5)
    p.add_argument("--line-search-tries", type=int, default=10)
    p.add_argument("--min-objective-improvement", type=float, default=1e-4)
...
def fit_geometry(X: np.ndarray, y: np.ndarray, layers: Sequence[int]) -> Tuple[dict, pd.DataFrame]:
    ...
    for i, L in enumerate(layers):
        ...
        rows.append({
            "layer": int(L), "fit_N": int(len(Xf)),
            "axis_H_dot_axis_V": float(np.dot(dH, dV)),
            "full_gap_H": gapH, "full_gap_V": gapV,
            "half_gap_H": halfH, "half_gap_V": halfV,
    ...
    def _hook(self, i: int, L: int):
        def hook(_m, _inp, out):
            ...
            if max(self.sub_pos + self.ref_pos) >= T:
                ...
                return None
            M = torch.as_tensor(self.geom[L]["natural_M"], device=h.device, dtype=h.dtype)
...
def smooth_objective(scores: Mapping[str, float], target: str, tau: float) -> float:
    vals = np.asarray([float(scores[r]) for r in REL if r != target], dtype=np.float64)
    m = float(np.max(vals / tau))
    lse = tau * (m + math.log(float(np.exp(vals / tau - m).sum())))
    return float(scores[target]) - float(lse)


def competitor_weights(scores: Mapping[str, float], target: str, tau: float):
    names = [r for r in REL if r != target]
    vals = np.asarray([float(scores[r]) / tau for r in names], dtype=np.float64)
    vals -= vals.max()
    w = np.exp(vals)
...
def apply_step(coords: np.ndarray, direction: np.ndarray, step: float) -> np.ndarray:
    return np.asarray(coords, dtype=np.float64) + float(step) * np.asarray(direction, dtype=np.float64)
...
def optimize_to_target(
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    if gen_pred == target:
        ...
        for step_idx in range(1, int(args.max_steps) + 1):
            if gen_pred == target:
                ...
                break
            obj0 = smooth_objective(scores, target, float(args.competitor_temperature))
            ...
            grad_scores = {r: sg[r][0] for r in REL}
            names, weights = competitor_weights(grad_scores, target, float(args.competitor_temperature))
            g = sg[target][1].copy()
            for r, w in zip(names, weights):
                g -= float(w) * sg[r][1]
            gn = float(np.linalg.norm(g.reshape(-1)))
            if gn < EPS:
            ...
            accepted = False
            for ls in range(int(args.line_search_tries)):
                step = float(args.step_natural) * (float(args.line_search_shrink) ** ls)
                cand = apply_step(coords, direction, step)
                cand_scores = all_scores(
                    model=model, decoder_layers=decoder_layers, prepared=prepared,
                    candidate_ids=candidate_ids, layers=layers, geom=geom,
                    coords=cand, reduction=args.sequence_score_reduction,
                )
                obj1 = smooth_objective(cand_scores, target, float(args.competitor_temperature))
                if obj1 >= obj0 + float(args.min_objective_improvement):
                    coords = cand
                    scores = cand_scores
                    accepted = True
                    break
            ...
            steps = step_idx
            run_gen = (
                step_idx % int(args.generation_check_every) == 0
                or argmax_rel(scores) == target
    ...
    # Always perform one final exact generation under the final spatial patch.
    ...
    return {
        ...
        "line_search_failed": bool(line_fail),
...
def run_pair(
    *, model_alias: str, dataset: str, model: Any, processor_or_backend: Any,
    ...
    print("Synthetic directions -> target GT selects head ID -> head prediction routes Synthetic residual H/V controller -> actual generation")
    ...
    meta = {
        ...
        "step_natural": float(args.step_natural),
        "max_steps": int(args.max_steps),
        "max_total_natural": None,
    }
