#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run filtered VG2 four-relation centroid comparison across models.
...
SCRIPT_VERSION = "vg2-centroid-generation-multimodel-v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", default="vg_two", choices=["vg_two"])
    ...
    parser.add_argument(
        "--step1-script",
        default="analyze_vg2_centroid_generation_step1_v1.py",
    )
    ...
    parser.add_argument(
        "--output-root",
        default="output/vg2_centroid_generation_multimodel_v1",
    )
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if missing:
                raise RuntimeError(
                    ...
                    "The output must be produced by VG2 Step-1 v1."
                )
...
def yes_no(value: Any) -> str:
    return "OK" if bool(value) else "WRONG"
...
def print_sample_results(
    ...
    """Print final oracle-selected predictions and running accuracy per sample."""
    running = {
        "similarity": 0,
        "headmean": 0,
        "best_head": 0,
    ...
    print("\n" + "#" * 120)
    print(f"PER-SAMPLE RESULTS: {model}")
    print("#" * 120)

    for index, row in enumerate(rows, 1):
        running["similarity"] += int(bool(row["similarity_correct"]))
        running["headmean"] += int(bool(row["headmean_attention_correct"]))
        running["best_head"] += int(bool(row["best_attention_head_correct"]))
        running["generation"] += int(bool(row["generation_correct"]))

        generation_prediction = row.get("generation_prediction") or "<invalid>"
        generation_text = one_line(row.get("generation_text"))

        print(f"\n[{index}/{total}] sid={row['sid']}")
        print(f"Question: {one_line(row.get('question'))}")
        print(f"GT: {row['gt']}")
        print(
            "Similarity centroid "
            f"[L{row['similarity_layer']}]: "
            f"pred={row['similarity_prediction']} | "
            f"{yes_no(row['similarity_correct'])} | "
            f"ACC={running['similarity']}/{index}="
            f"{running['similarity'] / index:.4f}"
        )
        print(
            "Mean-head attention centroid "
            f"[L{row['headmean_attention_layer']}]: "
            f"pred={row['headmean_attention_prediction']} | "
            f"{yes_no(row['headmean_attention_correct'])} | "
            f"ACC={running['headmean']}/{index}="
            f"{running['headmean'] / index:.4f}"
        )
        print(
            "Best-head attention centroid "
            f"[L{row['best_attention_head_layer']}"
            f"H{row['best_attention_head']}]: "
            f"pred={row['best_attention_head_prediction']} | "
            f"{yes_no(row['best_attention_head_correct'])} | "
            f"ACC={running['best_head']}/{index}="
            f"{running['best_head'] / index:.4f}"
        )
        print(f"Generation: {generation_text!r}")
        print(
            f"Generation pred={generation_prediction} | "
            f"{yes_no(row['generation_correct'])} | "
            f"ACC={running['generation']}/{index}="
            f"{running['generation'] / index:.4f}"
        )
...
def main() -> None:
    ...
    for model in models:
        ...
        try:
            ...
            print_sample_results(model, sample_part)
    ...
    print("\n" + "=" * 112)
    print("VG2 FOUR-RELATION CENTROID SUITE COMPLETE")
    print("=" * 112)
