...
# -*- coding: utf-8 -*-
"""
eval_multimodel_targetselected_head_spatial_control_fastmin_v4.py
...
For EACH model independently:
  ...
  B) Minimum-residual spatial actuator
     ...
     3. For the reader-selected target relation, compute target-vs-competitor
        relation-score margins and their exact gradients with respect to all
        2*7 spatial coordinates.  When the four answer surfaces are single-token,
        all relation scores share one prompt forward pass.
...
This controller does NOT use Synthetic half-gap scaling, a fixed
"natural" step, normalized-gradient ascent, or an inner nonlinear backtracking loop.
...
--overwrite to resume. The fast path shares one prompt forward across all four
relation logits and avoids empty_cache after every sample.
...
EPS = 1e-12
SCRIPT_VERSION = "multimodel-targetselected-head-spatial-control-fastmin-v4"
MODEL_ORDER = (
    "qwen2-2b",
    "qwen-3b",
    "qwen-7b",
...
def parse_args() -> argparse.Namespace:
    ...
    # Evaluation / fast minimum-distance optimization.
    ...
    p.add_argument("--max-steps", type=int, default=12, help="Maximum local relinearization steps")
    p.add_argument(
        "--decision-margin-eps", type=float, default=1e-4,
        help="Target relation-score margin required over every competing relation.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Reserved for compatibility; the projection solver does not use SVD.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Optional numerical trust cap on one local residual step; 0 disables it.",
    )
    p.add_argument(
        "--generation-check-every", type=int, default=1,
        help="Check actual generation after every N accepted minimum-boundary steps.",
    )
    p.add_argument(
        "--cuda-cleanup-every", type=int, default=25,
        help="Call gc/empty_cache every N target samples instead of every sample; 0 disables periodic cleanup.",
    )
    p.add_argument("--max-new-tokens", type=int, default=6)
    p.add_argument("--sequence-score-reduction", default="mean", choices=["mean", "sum"],
                   help="Used only by the multi-token fallback path.")
    return p.parse_args()
...
def optimize_to_target(
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    """Fast iterative minimum-residual boundary crossing.

    Each iteration evaluates all four relation scores at the current spatial
    state.  When every answer surface is one token, this costs ONE prompt
    forward pass; the same graph is reused for the three margin gradients.
    We solve the same local minimum-norm half-space problem as v3, apply that
    step directly, and relinearize on the next iteration.  There is no inner
    nonlinear line search and no Synthetic natural-gap step size.
    """
    ...
    if gen_pred == target:
        ...
        for step_idx in range(1, int(args.max_steps) + 1):
            # First get current scores and gradients.  On the fast path the
            # forward graph is shared across all three competitor gradients.
            if fast_token_ids is not None:
                # We need scores before deciding whether a gradient is necessary.
                # A single graph-building forward gives both; if already feasible
                # we simply do not request any backward pass.
                dev = prepared.batch["input_ids"].device
                delta = torch.zeros((len(layers), 2), device=dev, dtype=torch.float32, requires_grad=True)
                ...
                with SpatialPatch(
                    ...
                    if np.all(b <= float(args.qp_feas_tol)):
                        # The differentiable relation decision already prefers
                        # target.  Actual generation was checked immediately after
                        # the previous accepted step (or is the baseline), so a
                        # zero minimum step cannot change it further.
                        stop_reason = "relation_boundary_crossed_generation_mismatch"
                        del out
                        break
            ...
            if max_step > 0.0 and raw_step_norm > max_step:
                flat_step = flat_step * (max_step / raw_step_norm)
            ...
            # No expensive inner backtracking.  Apply the local minimum step,
            # let the nonlinear model move there, then relinearize next time.
            check_every = max(1, int(args.generation_check_every))
        ...
        # If the final accepted step was not generation-checked because of a
        # larger --generation-check-every, do exactly one final check.
        if steps > 0 and gen_pred != target and steps % max(1, int(args.generation_check_every)) != 0:
            gen_pred, gen_text = generate_with_coords(
                model_alias=model_alias, model=model, decoder_layers=decoder_layers,
                prepared=prepared, layers=layers, geom=geom, coords=coords, args=args,
            )
...
def summarize_pair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for mode, g in df.groupby("mode", sort=False):
        ...
        rows.append({
            ...
            "fraction_boundary_generation_mismatch": float(
                (g["stop_reason"] == "relation_boundary_crossed_generation_mismatch").mean()
            ),
...
def run_pair(
    *, model_alias: str, dataset: str, model: Any, processor_or_backend: Any,
    ...
    print("Synthetic directions -> target GT selects head ID -> head prediction sets target -> minimum residual H/V boundary step -> relinearize -> actual generation")
    ...
    meta = {
        ...
        "optimizer": "fast_iterative_minimum_residual_boundary_crossing",
