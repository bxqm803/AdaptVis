...
def parse_args():
    ...
    p.add_argument("--layers", default="0,1,2,3", help="Layers used by early intervention / head ablation.")
    ...
    p.add_argument("--out-csv", default="output/head_leave_one_out_first4_summary.csv")
    p.add_argument("--skip-full", action="store_true", help="Only run leave-one-out; not recommended because drops need full.")
    return p.parse_args()
...
def set_common_env(args, head_mode="all", heads=""):
    ...
    os.environ["ADAPTVIS_HEAD_MODE"] = str(head_mode)
    if heads:
        os.environ["ADAPTVIS_HEADS"] = str(heads)
    else:
        os.environ.pop("ADAPTVIS_HEADS", None)


def run_pass(args, wrapper, loader, prompts, answers, mode_name, base_records=None, head_mode="all", heads=""):
    set_common_env(args, head_mode=head_mode, heads=heads)

    print("\n" + "=" * 80)
    print("[RUN]", mode_name)
    print("[VARIANT]", os.environ.get("ADAPTVIS_ATTENTION_VARIANT"))
    print("[LAYERS]", os.environ.get("ADAPTVIS_LAYERS"))
    print("[HEAD_MODE]", os.environ.get("ADAPTVIS_HEAD_MODE"))
    print("[HEADS]", os.environ.get("ADAPTVIS_HEADS", ""))
    print("=" * 80)
...
def main():
    ...
    # Full early-head intervention baseline.
    full_summary, full_records = run_pass(
        args, wrapper, make_loader(), prompts, answers,
        mode_name="full_all_heads",
    ...
    for layer in layers:
        for head in range(args.num_heads):
            pair = f"{layer}:{head}"
            mode_name = f"except_L{layer}_H{head}"
            ...
            # Stream-save after every head so a long run can be inspected/resumed manually.
            fieldnames = [
                "mode", "layer", "head", "excluded_pair", "num_total",
