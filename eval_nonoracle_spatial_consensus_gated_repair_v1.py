...
# -*- coding: utf-8 -*-
"""
eval_nonoracle_spatial_consensus_gated_repair_v1.py

No-oracle, no-extra-trained-selector spatial repair.

Goal
====
Use the model's own mid-layer spatial representation to decide WHICH relation
should be trusted, and only then use the already validated spatial-only
optimizer to move the downstream decision toward that internally inferred
relation.

No GT is used for routing, gating, target selection, or optimization on an
evaluation sample.  GT is used only after the run to report accuracy/W2C/C2W.

Spatial selector
================
On a calibration split, for each controlled layer L, fit four class directions
from REAL-NoImage relation vectors:

    c_{L,r} = unit(mu_{L,r} - mu_L)

For an evaluation sample x, read its precomputed relation vector at each layer,
center it by mu_L, and score each relation by cosine:

    e_{L,r}(x) = cos(x_L - mu_L, c_{L,r})

Aggregate across L20-26:

    E_r(x) = mean_L e_{L,r}(x)

The non-oracle spatial target is

    r_spatial = argmax_r E_r.

Confidence diagnostics:
  spatial_margin = top1(E) - top2(E)
  agreement      = fraction of layers whose own argmax equals r_spatial

A repair candidate is created only when

    r_spatial != current generated decision.

Candidate repair
================
For a conflict sample, r_spatial replaces the oracle GT target.  The optimizer
may edit ONLY the per-layer spatial H/V subspaces.  At each step it maximizes

    J = S_target - tau * logsumexp({S_j/tau : j != target})

with a normalized local gradient and backtracking line search.  No model
parameter is trained or updated.

Gating / coverage evaluation
============================
All spatial-decision conflicts are optimized once.  Afterwards they are ranked
WITHOUT GT by a gate score (default: spatial_margin * agreement).  The script
then simulates repairing the top 0%,10%,...,100% of eligible conflicts and
reports accuracy, W2C, and C2W.  This gives the risk/coverage curve without
re-running the model for every threshold.

Important
=========
The calibration relation labels are used to define spatial prototypes/axes.
There is no evaluation-time oracle and no learned selector.  If a completely
label-free spatial router is desired later, the prototype construction itself
must be replaced.
"""
...
def main():
    ...
    geom, axis_df, layer_to_i = fit_spatial_geometry_and_selector(
        X, y, spatial_layers, fit_idx, source_layers
    )
    axis_df.to_csv(outdir / "spatial_axis_geometry.csv", index=False)
    sid_to_spatial_idx = {int(sid): i for i, sid in enumerate(spatial_sids.tolist())}

    two, all_meta, rec_by_sid = load_all_data(a)
    spatial_sid_set = set(sid_to_spatial_idx)
    ...
    try:
        ...
        print("\n" + "=" * 190)
        print("NON-ORACLE SPATIAL CONSENSUS GATED REPAIR")
        print("=" * 190)
        ...
        print("NO GT is used for target selection, conflict gating, or spatial optimization.")
        print("GT is read only after predictions exist, for evaluation metrics.")
        ...
        for m in tqdm(eval_meta, desc="EVAL non-oracle spatial repair"):
            ...
            image = batch = None
            try:
                sp = spatial_selector_for_sample(X[sid_to_spatial_idx[sid]], geom, source_layers)
        ...
        print("\n" + "=" * 190)
        print("NON-ORACLE SPATIAL SELECTOR")
        print("=" * 190)
        ...
        metadata = {
            ...
            "spatial_states_npz": a.spatial_states_npz,
            ...
            "note": "GT used only for post-hoc evaluation metrics; never for eval-time routing/target/optimization.",
        }
