...
# -*- coding: utf-8 -*-
"""
Find relation-free correlates of each sample's oracle-selected dynamic K24.

This script uses an existing dynamic-K24 run as LABELS, then asks:

    Can quantities already present in the sample's own forward pass predict
    which (layer, token) pairs entered that sample's K24?

It analyzes two levels:

A) TOKEN LEVEL
   For every candidate token at every source layer, compare K24 membership /
   oracle mediation against relation-free features such as:

     hidden-state:
       ||h_real||, ||h_gray||, ||real-gray||, relative delta,
       cosine(real, gray), norm change

     module activity:
       attention-output vector norm/delta
       MLP-output vector norm/delta

     same-sample similarity:
       cosine(delta_token, delta_last_at_source)
       cosine(delta_token, delta_last_at_late L32/L34/L35)
       cosine(delta_token, visual/subj/ref/relation/other delta centroids)

     attention:
       last-query -> candidate attention at source layer
       next-layer attention
       late-layer attention
       real-vs-gray attention change
       optional per-head source-layer attention

   For each scalar feature the script reports:
       pooled Spearman(feature, oracle mediation)
       mean within-sample Spearman(feature, mediation)
       pooled AUROC / AP for K24 membership
       mean within-sample AUROC
       mean K24 recall/Jaccard when ranking by the feature
       the same metrics with low-values ranked first

   The Top24 recovery uses the SAME global_unique constraint as the dynamic
   experiment: once a token position is chosen at one layer, that position
   cannot be chosen again at another source layer.

B) SAMPLE LEVEL
   Correlate global sample features (late delta norm, next-token entropy/margin,
   last-query attention entropy, etc.) against that sample's K24 composition:
       counts/fractions of L22:visual, L24:reference, L26:relation_words, ...

Important leakage labels
------------------------
Features are tagged:
  relation_free_current_forward
      potentially usable without LEFT/RIGHT/ON/UNDER routing

  oracle_gradient_diagnostic
      grad_norm from the original GT-writer attribution; diagnostic only

  circular_k24_diagnostic
      similarity to the TRUE K24 centroid; useful to understand geometry but
      impossible to use to discover K24 from scratch

The oracle mediation itself is the target and is never presented as a
relation-free predictor.

The script caches per-sample feature chunks, so an interrupted 440-sample run
can be resumed by rerunning without --overwrite.
...
CUDA_VISIBLE_DEVICES=0 python -u analyze_dynamic_k24_predictive_correlates_v1.py \
  ...
  --attention-head-scan \
  --output-dir output/qwen3b_coco_k24_predictive_correlates \
  --overwrite
"""
