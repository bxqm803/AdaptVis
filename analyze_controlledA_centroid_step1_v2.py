#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 1: attention-flow and swap-consistency observation.
...
SCRIPT_VERSION = "controlledA-centroid-step1-v2"

DEFAULT_PROMPT_FILE = Path(
    "prompts/Controlled_Images_A_with_answer_four_options.jsonl"
)

# Controlled-A uses on/under for the vertical axis. Geometrically,
# on is treated as above and under as below.
...
AUTO_LAYERS = {
    ...
    "qwen-3b": 24,
    "qwen-7b": 19,
    # Only affects the readable auxiliary report layer. All layers are still
    # evaluated when --layers all is used.
    "qwen2-vl-7b": 16,
}

# Qwen2-VL-7B is injected locally so this script does not require editing the
# Controlled-A extractor's SPECS table.
EXTRA_MODEL_SPECS = {
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dataset", default="controlled_A", choices=["controlled_A"])
    p.add_argument(
        "--prompt-jsonl",
        default=str(DEFAULT_PROMPT_FILE),
        help=(
            "Controlled-A standard question file. It is also passed to the "
            "Controlled-A record loader."
        ),
    )
    p.add_argument(
        "--controlled-module",
        ...
        help=(
            ...
            "script tries extract_controlled_relation_states_standalone and "
            "extract_controlledA_relation_states_standalone."
        ),
    )
    p.add_argument("--dataset-key", default="Controlled_Images_A")
    ...
    p.add_argument(
        ...
        default="all",
        help=(
            "Comma-separated zero-based decoder layers, for example "
            "12,18,24,30, or 'all'."
        ),
    )
    p.add_argument(
        "--report-layer",
        default="auto",
        help="Layer used for readable aggregate summaries, or 'auto'.",
    )
    ...
    p.add_argument(
        ...
        default=8,
        help=(
            "Generation length. Internally at least two tokens are generated "
            "so the first answer token can itself act as an attention query."
        ),
    )
    p.add_argument(
        "--temperature",
    ...
    p.add_argument(
        ...
        default=1,
        help=(
            "Retained for CLI compatibility. Normal generation is printed for "
...
def save_sample_arrays(
    ...
    *,
    ...
    arrays: Dict[str, Any] = {
        ...
        # One prediction per layer after averaging normalized attention maps
        # over all heads. These are the overall-attention centroid results.
...
def print_summary(summary: Dict[str, Any]) -> None:
    print("\n" + "=" * 100)
    print("STEP 1: ATTENTION FLOW AND SWAP CONSISTENCY")
    print("=" * 100)
    print(
...
def main() -> None:
    ...
    config = {
        ...
        "dataset_key": args.dataset_key,
        "controlled_module": module.__name__,
        "model": args.model,
        ...
        "generation_note": (
            "This Step-1 trace may force two decoding steps for answer-token "
            "routing. Do not use its generation accuracy as normal free "
            "generation; use the separate corrected generation run."
        ),
    }
