#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Evaluate counterfactual hidden-state amplification on COCO-two left/right.

This is a performance experiment built on the preceding image-flip activation
patching result.

For every left/right sample, the script runs the same question with:
...
left/right LM-head direction, using a centroid, training a probe, or changing
model weights.
...
Important:
- Only left/right records are evaluated.
- Horizontal flipping is fixed for the entire run; ground truth is NOT used to
...
VERSION = "coco-flip-state-amplification-v1"
...
def map_horizontal_scores(
    flipped_scores: Mapping[str, float],
) -> Dict[str, float]:
    return {
        relation: float(flipped_scores[HORIZONTAL_MAP[relation]])
        for relation in RELATIONS
    }
...
def report_text(
    *,
    ...
    lines = [
        "=" * len(header),
        "COCO LEFT/RIGHT COUNTERFACTUAL STATE AMPLIFICATION",
        (
...
def main() -> None:
    ...
    config = {
        ...
        "seed": args.seed,
        "audit": audit,
        "transform": "horizontal_flip_for_all_evaluated_samples",
        "evaluated_relations": ["left", "right"],
    ...
    try:
        for record in tqdm(
            ...
            try:
                ...
                if ground_truth not in ("left", "right"):
                    continue
                ...
                mapped_flip_scores = map_horizontal_scores(
                ...
                flipped_ground_truth = HORIZONTAL_MAP[ground_truth]
                ...
                for chunk_start in range(
                    ...
                    for condition, amplified_original, amplified_flipped in zip(
                        ...
                        mapped_amplified_flip_scores = map_horizontal_scores(
