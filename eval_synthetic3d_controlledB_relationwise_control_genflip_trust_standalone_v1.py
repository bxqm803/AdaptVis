...
# -*- coding: utf-8 -*-
"""
eval_synthetic3d_controlledB_relationwise_control_genflip_trust_standalone_v1.py
...
2) Actuator geometry: decoder residual object-pair Real-Gray states are
   extracted on the same Blender source at the seven controller layers. Each
   relation is fit as its own unit direction d_r = unit(mu_r - mu), with no
   left/right or front/behind pairing, no relation-axis construction, and no
   orthogonalization. For a chosen target relation t, each layer therefore has
   one scalar control coordinate along d_t (7D total over seven layers).
...
where delta_z_l = a_l d_{t,l} uses only the source-fitted direction of the
chosen target relation t at each layer. For the chosen relation target, the
script computes target-vs-competitor relation-score
...
EPS = 1e-12
SCRIPT_VERSION = "synthetic3d-to-controlledB-relationwise-controller-v1"
...
def parse_args() -> argparse.Namespace:
    ...
    # Blender 3D Synthetic-600 source. Controlled-B uses L/R/front/behind.
    ...
    # Relation-wise controller geometry / optimization.
...
# =============================================================================
# Synthetic-3D residual relation-wise geometry
# =============================================================================
...
def fit_geometry(X: np.ndarray, y: np.ndarray, layers: Sequence[int]) -> Tuple[dict, pd.DataFrame]:
    """Fit one independent Synthetic-3D relation direction per class and layer.

    For relation r at layer l:

        d_{r,l} = unit(mu_{r,l} - mu_l)

    where mu_l is the mean source residual state at that layer.  Relations are
    *not* paired into horizontal/depth axes and the relation directions are not
    orthogonalized.  During control, the selected target relation t contributes
    exactly one unit direction d_{t,l} per layer, so the seven-layer controller
    has seven scalar coordinates rather than 14 axis coordinates.

    Because each d_{r,l} is unit norm, |a_l| is exactly the L2 norm of the
    residual edit a_l d_{r,l} at that layer.  Thus the minimum-coordinate-norm
    QP remains a minimum-residual-norm QP without any extra metric correction.
    """
    geom: Dict[int, dict] = {}
    rows: List[dict] = []
    ...
    for i, L in enumerate(layers):
        ...
        relation_dirs = {
            r: unit(means[r] - center).astype(np.float32)
            for r in REL
        }
        ...
        # Diagnostics only: report similarities without modifying the vectors.
        for a_i, a in enumerate(REL):
            for b in REL[a_i + 1:]:
                row[f"cos_{a}__{b}"] = float(
                    np.dot(relation_dirs[a].astype(np.float64),
                           relation_dirs[b].astype(np.float64))
                )
        geom[int(L)]["pairwise_cosines"] = {
            k: float(v) for k, v in row.items() if str(k).startswith("cos_")
        }
        rows.append(row)
...
class SpatialPatch:
    """Patch object-token residual states along ONE target-relation direction.

    At layer l the edit is

        delta_z_l = a_l * d_{target,l}

    with d_{target,l} fitted directly from the Synthetic-3D class mean.
    No relation pair, axis, span, or orthogonalization is used.
    """
    def __init__(
        self, *, decoder_layers: Sequence[Any], layers: Sequence[int], geom: Mapping[int, Any],
        target: str, sub_pos: Sequence[int], ref_pos: Sequence[int], coords: torch.Tensor,
    ):
        self.handles = []
        self.layers = list(map(int, layers))
        self.geom = geom
        self.target = str(norm_rel(target))
        if self.target not in REL:
            raise ValueError(f"Bad relation-wise control target={target!r}")
        self.sub_pos = list(map(int, sub_pos))
        self.ref_pos = list(map(int, ref_pos))
        self.coords = coords
    ...
    def _hook(self, i: int, L: int):
        def hook(_m, _inp, out):
            ...
            if max(self.sub_pos + self.ref_pos) >= T:
                ...
                return None
            d = torch.as_tensor(
                self.geom[L]["relation_dirs"][self.target],
                device=h.device,
                dtype=h.dtype,
            )
            # coords is [n_layers, 1]; each row is the scalar a_l.
            a = self.coords[i].reshape(-1)[0].to(device=h.device, dtype=h.dtype)
            dx = a * d
            y = h.clone()
...
def fast_relation_scores_and_grads(
    *, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    delta = torch.zeros((len(layers), 1), device=dev, dtype=torch.float32, requires_grad=need_grads)
    base = torch.as_tensor(base_coords, device=dev, dtype=torch.float32)
    coords = base + delta
    with SpatialPatch(
        decoder_layers=decoder_layers, layers=layers, geom=geom, target=target,
    ...
    *, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    for r in REL:
        sg[r] = score_and_grad(
            ...
            base_coords=base_coords, control_target=target, reduction=reduction,
        )
...
def score_and_grad(
    *, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    base_coords: np.ndarray, control_target: str, reduction: str,
) -> Tuple[float, np.ndarray]:
    ext = extend_batch(prepared.batch, answer_ids)
    dev = prepared.batch["input_ids"].device
    delta = torch.zeros((len(layers), 1), device=dev, dtype=torch.float32, requires_grad=True)
    base = torch.as_tensor(base_coords, device=dev, dtype=torch.float32)
    coords = base + delta
    with SpatialPatch(
        decoder_layers=decoder_layers, layers=layers, geom=geom, target=control_target,
        sub_pos=prepared.sub_pos, ref_pos=prepared.ref_pos, coords=coords,
    ):
...
def score_only(
    *, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    answer_ids: Sequence[int], layers: Sequence[int], geom: Mapping[int, Any],
    coords: np.ndarray, control_target: str, reduction: str,
) -> float:
    ext = extend_batch(prepared.batch, answer_ids)
    dev = prepared.batch["input_ids"].device
    ...
    *, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    return {
        r: score_only(
            ...
            control_target=control_target, reduction=reduction,
        )
        for r in REL
    }
...
def generate_with_coords(
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    coords: np.ndarray, args: argparse.Namespace,
    ...
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    for _ in range(n):
        ...
        pred, text = generate_with_coords(
            ...
            prepared=prepared, layers=layers, geom=geom, target=target, coords=mid, args=args,
        )
        checks += 1
        if pred == target:
            high = mid
...
def optimize_to_target(
    *, model_alias: str, model: Any, decoder_layers: Sequence[Any], prepared: Prepared,
    ...
    direction in the L relation-wise control coordinates.  The first goal is to cross the
    ...
    that segment.
    """
    coords = np.zeros((len(layers), 1), dtype=np.float64)
    ...
    if gen_pred == target:
        ...
        for step_idx in range(1, int(args.max_steps) + 1):
            # Current relation scores + exact margin gradients in the target-relation
            ...
            step = flat_step.reshape(len(layers), 1)
            ...
            proposed_pred, proposed_text = generate_with_coords(
                model_alias=model_alias, model=model, decoder_layers=decoder_layers,
                prepared=prepared, layers=layers, geom=geom, target=target, coords=proposed_coords, args=args,
            )
...
def run_pair(
    *, model_alias: str, dataset: str, model: Any, processor_or_backend: Any,
    ...
    print(f"Synthetic-3D relation-wise geometry layers={list(layers)} | eval N={len(target_rows)}")
    if fast_token_ids is not None:
    ...
    print("Synthetic-3D independent relation directions -> one scalar/layer along target relation -> minimum residual step -> deepen score margin until ACTUAL generation flips -> trust-cap each local step -> binary-refine final step")
    ...
    for row in pbar:
        ...
        try:
            ...
            zero = np.zeros((len(layers), 1), dtype=np.float64)
            base_pred, base_text = generate_with_coords(
                model_alias=model_alias, model=model, decoder_layers=decoder_layers,
                prepared=prepared, layers=layers, geom=geom, target=head_pred, coords=zero, args=args,
            )
    ...
    meta = {
        ...
        "residual_geometry_source": "Blender Synthetic-3D independent left/right/front/behind class directions",
        "geometry_control": "gray",
        "controller_layers": list(map(int, layers)),
        "controller_dim": int(len(layers)),
        "optimizer": "iterative_minimum_residual_generation_flip_with_trust_region",
        "spatial_basis": "independent Synthetic-3D relation direction d_r=unit(mu_r-mu); no axis pairing or orthogonalization",
...
def main() -> None:
    ...
    print(
        f"[PROTOCOL] source=Blender Synthetic-3D independent LR/front/behind directions | "
    ...
    for model_alias in MODEL_ORDER:
        ...
        try:
            ...
            if len(layers) != 7:
                raise RuntimeError(
                    f"{model_alias}: controller must use exactly 7 layers for 7D relation-wise comparability; got {layers}"
                )
            ...
            # B) Source-only decoder residual relation-wise actuator geometry.
            ...
            geom, geometry_df = fit_geometry(Xg, yg, layers)
            model_geom_dir = geom_root / model_alias
            model_geom_dir.mkdir(parents=True, exist_ok=True)
            geometry_df.to_csv(model_geom_dir / "synthetic3d_relationwise_geometry.csv", index=False)
    ...
    print("\n" + "=" * 180)
    print("FINAL SYNTHETIC-3D RELATION-WISE -> CONTROLLED-B CONTROL SUMMARY")
    print("=" * 180)
    if summaries:
