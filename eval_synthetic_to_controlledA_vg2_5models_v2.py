...
"""
eval_synthetic_to_controlledA_vg2_5models_v2.py
...
def load_vg2_lr_sixoption(args):
    """Load aligned original six-option VG two-object data, then keep left/right only."""
    ...
    actual_ids = sorted(prompts)
    expected_ids = list(range(len(prompts)))
    if actual_ids != expected_ids:
        raise RuntimeError("VG prompt ids are not sequential 0..N-1")
    if len(data) != len(prompts):
        raise RuntimeError(f"VG data/prompt length mismatch: data={len(data)} prompts={len(prompts)}")
    image_root = causal.resolve_vg_image_root(args, data)
    raw_counts = Counter(prompts[sid]["relation"] for sid in sorted(prompts))
    records = []
    skipped = Counter()
    missing_images = []
    for sid, item in enumerate(data):
        ...
        if image_path is None:
            ...
            continue
        records.append({
            "sid": sid, "dataset": "vg2", "image_id": str(image_id),
            "image_path": str(image_path), "subject": prompt["subject"],
            "reference": prompt["reference"], "relation": relation,
            "question_text": prompt["question_text"], "raw_question": prompt["raw_question"],
            "pair_group": " || ".join(sorted([prompt["subject"].lower(), prompt["reference"].lower()])),
        })
    counts = Counter(r["relation"] for r in records)
    print(f"[VG2-LR six-option] data={data_path} | prompt={prompt_path} | image_root={image_root}")
    print(f"[VG2-LR six-option] source_rows={len(data)} | prompts={len(prompts)} | raw_normalized_prompt_relations={dict(raw_counts)}")
    print(f"[VG2-LR six-option] loaded_LR={len(records)} | relations={dict(counts)} | skipped={dict(skipped)}")
    if missing_images:
        print("[VG2-LR six-option] missing image examples=" + ", ".join(missing_images))
    missing_lr = [r for r in ("left", "right") if counts[r] == 0]
    if missing_lr:
        raise RuntimeError(f"VG2-LR loader lacks required relations {missing_lr}")
    return records
...
def load_target_dataset(dataset, args):
    ...
    if dataset == "vg2_lr":
        args.dataset = "vg2"
        records = load_vg2_lr_sixoption(args)
...
def build_target_swapped_question(record, dataset):
    if dataset == "vg2_lr":
        return (
            f"Where is the {record['reference']} in relation to the {record['subject']}? "
            "Answer with left, right, front, behind, above or below."
        )
    return cent.build_swapped_question(record["subject"], record["reference"])
...
def target_centroid_predict(
    ...
    swapped_question = build_target_swapped_question(
        record,
        dataset,
    )
