#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run the requested COCO centroid-versus-generation comparison.
...
For each model, this script runs:
    analyze_coco_similarity_head_generation_step1_v2.py

It reports exactly three headline results:

1. Best hidden-similarity centroid accuracy and its decoder layer.
   This uses the original/swap-aligned average similarity centroid.

2. Best single attention-head centroid accuracy and its layer/head.
   This uses the original/swap-aligned average attention centroid.

3. Normal model generation accuracy.
   Invalid or unparseable generations count as wrong.

During Step 1, every normal generation is printed as:
...
No train30/test70 split, top-k head ensemble, or fine-tuning is used.

Important:
- Selecting the best similarity layer and best attention head from the full
  evaluation set uses GT labels and is therefore a diagnostic oracle selection.
- The model itself is frozen and receives no training or parameter update.
"""
...
SCRIPT_VERSION = "coco-similarity-head-generation-multimodel-v2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--step1-script",
        default="analyze_coco_similarity_head_generation_step1_v2.py",
    )
    parser.add_argument(
        "--python",
    ...
    parser.add_argument(
        "--report-layer",
        default="auto",
        help="Only affects Step 1's auxiliary report; final selection scans all layers.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional smoke-test limit. Omit for the full COCO-two set.",
    )
    parser.add_argument(
        "--output-root",
        default="output/coco_similarity_head_generation_multimodel",
    )
    parser.add_argument(
        "--skip-step1",
        action="store_true",
...
def main() -> None:
    ...
    if missing_models:
        print(
            ...
            "extract_two_object_relation_states.SPECS and will be skipped: "
            + ", ".join(missing_models)
        )
