...
GOLD spatial-answer probability read directly from the FINAL prompt token.
...
For each train/test split and each decoder block L:
  ...
  3) on the SAME image+question, run one normal forward pass only;
  4) read outputs.logits[0, -1], i.e. the LM-head distribution produced by
     the final prompt token for the NEXT token;
  5) measure the GOLD relation probability / conditional probability / logit
     margin for left/right/on/under.

Internal dataset labels:
    left, right, above, below
Surface answer labels:
    left, right, on, under
with above->on and below->under.

The last-token score reports:
  * last_token_gt_sum_probability:
  ...
  * last_token_gt_max_variant_probability:
      max probability over those variants;
  * last_token_gt_conditional_probability:
      gold category mass normalized over the four answer-category masses;
  * last_token_gt_logit_margin:
      best gold-variant logit minus best competing-category variant logit.

Main held-out correlations are layer-by-layer:
    residual_gold_cosine  vs last_token_gt_sum_probability
    residual_gold_margin  vs last_token_gt_logit_margin
plus the other probability variants.

No generation is performed.

Example:
    CUDA_VISIBLE_DEVICES=0 python analyze_coco_residual_similarity_vs_last_token_probability_v1.py \
      --model qwen-3b \
      --data-root data \
      --device cuda:0 \
      --train-ratio 0.15 \
      --repeats 5 \
      --output-dir output/coco_residual_vs_last_token_prob/qwen-3b \
      --states-npz output/coco_residual_vs_gold_prob/qwen-3b/states/correct_minus_noimage.npz
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--states-npz",
        default=None,
        help="Optional existing Correct-NoImage states .npz to reuse. "
             "If omitted, states are stored under OUTPUT_DIR/states/.",
    )
...
def extract_last_token_probabilities(
    *,
    args: argparse.Namespace,
    model: Any,
    processor: Any,
    ...
    """Read the NEXT-token answer distribution from the final prompt token.

    outputs.logits[0, -1] is already the model's final-normalized LM-head output
    for the last input token, i.e. the distribution used to predict the first
    generated token. No autoregressive generation is involved.
    """
    if out_csv.exists() and not args.overwrite_generation:
        print(f"[reuse last-token probabilities] {out_csv}")
    ...
    for sid in tqdm(sids, desc=f"{args.model}:last-token-prob", dynamic_ncols=True):
        ...
        try:
            ...
            last_input_token_id = int(batch["input_ids"][0, -1].item())
            last_input_token_piece = safe_decode(
                processor.tokenizer,
                [last_input_token_id],
                skip_special_tokens=False,
            )
            ...
            if not hasattr(outputs, "logits") or outputs.logits is None:
                raise RuntimeError("Forward output does not contain logits")
            ...
            pred_surface = str(scored["answer_step_prediction"])
            pred_internal = SURFACE_TO_INTERNAL[pred_surface]
            correct = bool(pred_internal == gt)

            full_probs = torch.softmax(logits.float(), dim=-1)
            vocab_argmax_id = int(torch.argmax(logits).item())
            vocab_argmax_prob = float(full_probs[vocab_argmax_id].item())
            vocab_argmax_piece = safe_decode(
                processor.tokenizer,
                [vocab_argmax_id],
                skip_special_tokens=False,
            )
            ...
            row: Dict[str, Any] = {
                ...
                "last_input_token_id": last_input_token_id,
                "last_input_token_piece": last_input_token_piece,
                "last_token_prediction_surface": pred_surface,
                "last_token_prediction": pred_internal,
                "last_token_correct": correct,
                "last_token_gt_sum_probability": float(sum_probs[gt_i]),
                "last_token_gt_max_variant_probability": float(max_probs[gt_i]),
                "last_token_gt_conditional_probability": float(cond[gt_i]),
                "last_token_gt_logit_margin": gt_margin,
                "vocab_argmax_token_id": vocab_argmax_id,
                "vocab_argmax_token_piece": vocab_argmax_piece,
                "vocab_argmax_probability": vocab_argmax_prob,
            }
            for i, rel in enumerate(SURFACE_RELATIONS):
                row[f"last_token_sum_prob_{rel}"] = float(sum_probs[i])
                row[f"last_token_max_prob_{rel}"] = float(max_probs[i])
                row[f"last_token_cond_prob_{rel}"] = float(cond[i])
                row[f"last_token_max_logit_{rel}"] = float(max_logits[i])
            rows.append(row)

            if not args.quiet_samples:
                print(
                    f"[{args.model}] sid={sid:4d} gt={gt_surface:5s} "
                    f"pred4={pred_surface:5s} "
                    f"Pgt={sum_probs[gt_i]:.6g} "
                    f"Pgt|4={cond[gt_i]:.6f} "
                    f"margin={gt_margin:+.4f} "
                    f"correct={correct}",
                    flush=True,
                )
            ...
            row = {
                ...
                "gt_surface": gt_surface,
                "score_found": False,
                "last_token_prediction": None,
                "last_token_correct": False,
                "error_type": type(exc).__name__,
                "error": str(exc),
            }
            rows.append(row)
    ...
    pd.DataFrame(errors).to_csv(
        out_csv.parent / "last_token_probability_errors.csv", index=False
    )
    return frame
...
def analyze_heldout(
    *,
    ...
    layers: Sequence[int],
    sids: Sequence[int],
    last_token_scores: pd.DataFrame,
    splits: Sequence[Tuple[np.ndarray, np.ndarray]],
    out_dir: Path,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    score_df = last_token_scores.copy()
    score_df["sid"] = score_df["sid"].astype(int)
    score_by_sid = {int(row.sid): row for row in score_df.itertuples(index=False)}
    ...
    for rep, (train_idx, test_idx) in enumerate(splits):
        for li, layer in enumerate(layers):
            ...
            for j, global_i in enumerate(test_idx.tolist()):
                ...
                if g is None:
                    row["score_found"] = False
                else:
                    for name in [
                        "score_found",
                        "last_token_prediction",
                        "last_token_prediction_surface",
                        "last_token_correct",
                        "last_input_token_id",
                        "last_input_token_piece",
                        "last_token_gt_sum_probability",
                        "last_token_gt_max_variant_probability",
                        "last_token_gt_conditional_probability",
                        "last_token_gt_logit_margin",
            ...
            found = local[local["score_found"] == True].copy()  # noqa: E712
            groups = {
                "all": found,
                "last_token_correct": found[found["last_token_correct"] == True],  # noqa: E712
                "last_token_wrong": found[found["last_token_correct"] == False],  # noqa: E712
            }
...
def main() -> None:
    ...
    states_path = (
        Path(args.states_npz)
        if args.states_npz is not None
        else out_dir / "states" / "correct_minus_noimage.npz"
    )
    last_token_csv = out_dir / "last_token_probabilities.csv"
    ...
    try:
        ...
        last_token_scores = extract_last_token_probabilities(
            args=args,
            model=model,
            processor=processor,
        ...
        found = last_token_scores[last_token_scores["score_found"] == True].copy()  # noqa: E712
        last_acc = (
            float(found["last_token_correct"].astype(float).mean())
            if len(found) else float("nan")
        )
        print("\n=== FINAL PROMPT-TOKEN LM HEAD ===")
        print(
            f"score_found={len(found)}/{len(last_token_scores)} "
            f"four-way_acc={100.0*last_acc:.2f}%"
        )
        if len(found):
            print(
                f"mean P(gold full-vocab mass)="
                f"{found['last_token_gt_sum_probability'].mean():.6f}"
            )
            print(
                f"mean P(gold | four relations)="
                f"{found['last_token_gt_conditional_probability'].mean():.6f}"
            )
        ...
        margin_corr = corr_summary[
            (corr_summary["group"] == "all")
            & (corr_summary["x_metric"] == "residual_gt_margin")
            & (corr_summary["y_metric"] == "last_token_gt_logit_margin")
        ].copy()
        if len(margin_corr):
            best_ms = margin_corr.loc[margin_corr["spearman_r_mean"].abs().idxmax()]
            best_mp = margin_corr.loc[margin_corr["pearson_r_mean"].abs().idxmax()]
            print("\n=== MARGIN CORRELATION: residual margin vs LM logit margin ===")
            print(
                f"max |Spearman|: L{int(best_ms['layer'])} "
                f"rho={best_ms['spearman_r_mean']:.4f} "
                f"±{best_ms['spearman_r_std']:.4f}"
            )
            print(
                f"max |Pearson| : L{int(best_mp['layer'])} "
                f"r={best_mp['pearson_r_mean']:.4f} "
                f"±{best_mp['pearson_r_std']:.4f}"
            )
            at_best_m = margin_corr[margin_corr["layer"] == int(best_acc["layer"])]
            if len(at_best_m):
                row = at_best_m.iloc[0]
                print(
                    f"at best-ACC L{int(best_acc['layer'])}: "
                    f"Pearson={row['pearson_r_mean']:.4f}, "
                    f"Spearman={row['spearman_r_mean']:.4f}"
