...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--transition-after", type=float, default=26.5,
                   help="Draw a subtle vertical marker here. Set <0 to disable.")
    p.add_argument("--show-transition-label", action="store_true",
    ...
    p.add_argument("--output-dir", default="output/intro_hiddenstate_decision_pair_v2/paper_figure")
...
def rows_for_sid(rows: List[Dict[str, str]], sid: int) -> List[Dict[str, str]]:
...
def main() -> None:
    ...
    validate_pair(cr, wr, a.target_relation, a.opposite_relation)

    c_layers = np.asarray([int(r["layer"]) for r in cr], dtype=int)
    w_layers = np.asarray([int(r["layer"]) for r in wr], dtype=int)
    if not np.array_equal(c_layers, w_layers):
        raise RuntimeError("The two samples do not share the same layer set.")

    c_gap = np.asarray([float(r["prob_gap"]) for r in cr], dtype=float)
    w_gap = np.asarray([float(r["prob_gap"]) for r in wr], dtype=float)

    outdir = Path(a.output_dir)
    outdir.mkdir(parents=True, exist_ok=True)

    # Paper-style sizing: wide enough for a two-column figure but compact vertically.
    fig, ax = plt.subplots(figsize=(7.3, 3.65))

    ax.plot(
        c_layers, c_gap,
        marker="o", markersize=5.0,
        linewidth=2.2,
        label=arrow_label(a.target_relation, a.target_relation),
    )
    ax.plot(
        w_layers, w_gap,
        marker="o", markersize=5.0,
        linewidth=2.2,
        linestyle="--",
        label=arrow_label(a.target_relation, a.opposite_relation),
    )

    # Neutral reference line.
    ax.axhline(0.0, linestyle="--", linewidth=1.25, alpha=0.65)

    # Optional transition marker. Kept subtle because mechanism details belong in the main text.
    if a.transition_after >= 0:
        ax.axvline(a.transition_after, linestyle=":", linewidth=1.25, alpha=0.55)
    ...
    ax.set_ylabel("Mapped-option preference", fontsize=14)

    # Use even-numbered layers for a cleaner paper figure.
    xticks = [int(x) for x in c_layers if int(x) % 2 == 0]
    ax.set_xticks(xticks)
    ax.tick_params(axis="both", labelsize=11.5)

    # Very light grid; keeps the sharp late bifurcation readable without looking like a debug plot.
    ax.grid(True, axis="both", linewidth=0.55, alpha=0.20)

    # Short, semantic legend; sample IDs belong in the caption, not the plot.
    leg = ax.legend(
        loc="upper left",
        frameon=False,
        fontsize=11.5,
    ...
    print(f"[OK] correct sid : {a.correct_sid}")
    print(f"[OK] wrong sid   : {a.wrong_sid}")
    print(f"[OK] PNG         : {png}")
    print(f"[OK] PDF         : {pdf}")
    print()
    print("Suggested caption definition:")
    print(
        f"Mapped-option preference = P(pi({a.target_relation})) - P(pi({a.opposite_relation})), "
        "where probabilities are obtained by applying the model's final normalization and LM head "
        "to the prompt-final hidden state at each decoder layer and softmaxing over A/B/C/D."
    )
